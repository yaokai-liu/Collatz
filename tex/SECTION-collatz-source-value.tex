...
Given $b \in\widehat{\mathbb{N}}_\beta$, define $\chr b = \min_{v\in\mathbb{N}_{\ge1}}\{\beta^{v}b: \alpha\mid(\beta^{v}b-\gamma)\}$ called the Collatz cahracter of $b$. If $\{\beta^{v}b: \alpha\mid(\beta^{v}b-\gamma)\} = \varnothing$, we define $\chr b = \infty$. All numbers that Collatz character is $\infty$ are denoted as $\mathbb{S}$.
And given $B \subset \widehat{\mathbb{N}}_\beta$, we denote that $\chr B = \{ \chr b \mid b\in B \} - \{ \infty \}$.
\end{definition}
\begin{proposition}
    ...
    \end{aligned}\right.$
    If $\alpha \nmid \gamma$, then $\forall b\in\alpha\widehat{\mathbb{N}}_\beta$, $\chr b = \infty$.
\end{proposition}
This proposition states that whether $\alpha \mid \gamma$, or $\alpha \nmid \gamma$ but $b \in \alpha\widehat{\mathbb{N}}_\beta$, these are all trivial cases. And if $\chr b = \infty$, we call them primitive sources, and denote all primitive sources as $\mathbb{S}$, they will be discussing in \cref{sec:distribution-of-collatz-primitive-source}.
\begin{proposition}
...
\begin{proof}
    ...
    \end{align*}
\end{proof}
\begin{corollary}~\label{cor:form-of-1-step-source-value-set}

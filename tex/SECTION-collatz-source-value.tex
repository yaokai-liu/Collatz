...
\begin{proof}
    ...
    From \cref{prop:complement-coprime-with-alpha-and-beta}, we have $\gcd(\alpha, \mathfrak{V}_{k}) = 1$, the divisibility condition simplifies to
    \[\alpha^{k} \mid \gamma\cdot(\beta^{s} - 1).\]
    When $\alpha^{k} \nmid \gamma$.
    By the fundamental theorem of arithmetic, we can derive that $\alpha = \prod_{p \in \mathbb{P}}^{p \mid \alpha} \raf_{p}\alpha$. Thus, the global condition is equivalent to a set of local conditions for each prime $p \mid \alpha$ we have

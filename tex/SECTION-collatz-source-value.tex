...
\begin{align*}
    ...
    \mathbb{V}_n(B) &= \bigcup_{b\in B}\left\{ \bm{v} \in \mathbb{N}^{n} \mid f_{n}(\bm{v};b) \in \widehat{\mathbb{N}}_{\beta} \right\}.
\end{align*}
And denote
\[\mathbb{F}(B) = B\cup\bigcup_{b\in B}\{ a\in \widehat{\mathbb{N}}_{\beta} \mid a\rightsquigarrow b \} = \bigcup_{n = 0}^{\infty}\mathbb{F}_{n}(B)\]
...
\begin{definition}~\label{def:collatz-character}\nomenclature{$\chr b$}{Collatz character of $b$, \cref{def:collatz-character}}
    Given $b \in\widehat{\mathbb{N}}_\beta$, define $\chr b = \min_{v\in\mathbb{N}_{\ge1}}\{\beta^{v}b: \alpha\mid(\beta^{v}b-\gamma)\}$ called the Collatz cahracter of $b$. If $\{\beta^{v}b: \alpha\mid(\beta^{v}b-\gamma)\} = \varnothing$, we define $\chr b = 0$.
\end{definition}
\begin{proposition}
...
\end{proposition}
This proposition states that whether $\alpha \mid \gamma$, or $\alpha \nmid \gamma$ but $\beta \in \alpha\widehat{\mathbb{N}}_\beta$, these are all trivial cases. And if $\chr b = 0$, we call them primitive sources, and denote all primitive sources as $\mathbb{S}$, it will be discussing in \cref{sec:distribution-of-def:collatz-primitive-source}.
\begin{proposition}
    $\chr(a + m\alpha) = \chr a + m\alpha\cdot\pow(\beta\mid\chr a)$.
\end{proposition}
From \cref{thm:vary-rule} and \cref{thm:the-periodic-rule-of-reduce-sequence}, we can derive
\begin{theorem}~\label{thm:layers-of-reduce-space-and-source-value-set}
    \begin{align*}
        ...
        &= \bigcup_{a\in \mathbb{F}_{n}(b) - \mathbb{S}} \left\{\left.\frac{1}{\alpha}\left(\beta^{k\Phi_1}\cdot\chr a - \gamma\right)\right\vert k\in \mathbb{N}\right\},\\
        \mathbb{V}_{n+1}(b) &= \bigcup_{a\in \mathbb{F}_{n}(b) - \mathbb{S}} \left\{ (k\Phi_1 +  \chr a, \bm{v}) \mid  k\in \mathbb{N}, \bm{v} \in \mathbb{V}_{n}(b) \right\}\\

...
We can observe that if $\Delta\bm{v}$ is only $1$-dimensional, then $\nabla\cdot\mathfrak{V}_1$ and $\nabla\cdot\mathfrak{v}_1$ do not depend on $\bm{v}$ in \cref{thm:vary-rule}; they depend only on $\Delta\bm{v}$. Therefore, if $\Delta\bm{v} = (s\Phi_1)$, we can always perform variational analysis with an integer number of hops. Thus, we can consider the minimum number of hops, $\frac{\gamma}{\alpha}\cdot\nabla\cdot\mathfrak{v}_1$, and denote it as
\[ \nabla_1 = \frac{\gamma}{\alpha}\cdot\nabla\cdot\mathfrak{v}_1 = \frac{\gamma}{\alpha}\cdot(\beta^{\Phi_1} - 1). \]
We can find it will play a crucial role in later sections.
\begin{definition}
...
\begin{proposition}
    For simple Collatz system, we have \begin{align*}
        &\Psi(\alpha, \beta, \gamma) = \frac{1}{\raf_{\alpha}(\beta^{\ord_{\alpha}\beta} - 1)}, \\
        &\Phi(\alpha, \beta, \gamma; k) = \ord_{\alpha}\beta \cdot \frac{\alpha^{k}}{\raf_{\alpha}(\beta^{\ord_{\alpha}\beta} - 1)}.
    \end{align*}
    And we have $\Phi_k = \ord_{p}q \cdot p^{k-m}$, here $m = \val_{\alpha}(\beta^{\ord_{\alpha}\beta} - 1)$.
\end{proposition}

...
\begin{definition}~\label{def:collatz-character}
    Denote $\chr b = \min_{v\in\mathbb{N}_{\ge1}}\{\beta^{v}b: \alpha\mid(\beta^{v}b-\gamma)\}$ called the Collatz cahracter of $b$.
\end{definition}
...
\begin{theorem}~\label{thm:layers-of-reduce-space-and-source-value-set}
    ...
    \end{align*}
    Here the $\mathbb{S}$ is all primitive source defined as \cref{collatz-primitive-source}.
\end{theorem}
...
\end{proof}
\begin{corollary}
    Let $B = \left\{\beta^{k\Phi}\mid k\in\mathbb{N}\right\}$. $\forall b\in\widehat{\mathbb{N}}_\beta - \mathbb{S}$, we have
...
\end{definition}
\begin{proposition}
    $a_{i} \cdot \beta^{\Phi_1\cdot\rdu\val(a;i) + s_i} = \alpha \cdot a_{i-1} + \gamma$.
\end{proposition}

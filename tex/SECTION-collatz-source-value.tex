...
\begin{example}
    Suppose $q \in\widehat{\mathbb{N}}_{2}$ that $q > 1$, then we have \begin{align*}
        ...
        &\Phi(q, 2, 1; k) = q^{k}\cdot\prod_{p\in\Pi(\mathbb{P}\mid q)}\frac{1}{\raf_{p}(2^{\ord_{p}2} - 1)} \cdot \lcm_{p\in\mathbb{P}}^{p\mid q}\{\ord_{p}2\}.
    \end{align*}
...
\begin{definition}
We denote $\nabla_1 = \frac{\nabla\cdot\mathfrak{v}_1}{\alpha} = \frac{\beta^{\Phi_1} - 1}{\alpha}$\label{def:basic-fill part-of-adjustment}. It's a broad form of the Fermat quotient, here we call it the basic complement of adjustment.
...
\end{theorem}
\begin{proof}
    For simple Collatz system, we have $\alpha^{k} \mid (\beta^{\Phi_k} - 1)$. Then
    \begin{align*}
        \frac{\beta^{\Phi_k} - 1}{\alpha^{k}} &= \frac{1}{\alpha^{k}}((\beta^{\Phi_1})^{\alpha^{k-1}} - 1) && \\
        &= \frac{1}{\alpha^{k}}((1 + \alpha\nabla_1)^{\alpha^{k-1}} - 1) && \\
        &= \frac{1}{\alpha^{k}}\sum_{i=1}^{\alpha^{k-1}}\binom{\alpha^{k-1}}{i} \alpha^{i}\nabla_1^{i} && \\
        &= \nabla_1 + \frac{1}{\alpha^{k}}\sum_{i=2}^{\alpha^{k-1}}\binom{\alpha^{k-1}}{i} \alpha^{i}\nabla_1^{i} && \\
        &\equiv \nabla_1.  &&\pmod{\alpha}
    \end{align*}
    Then from \cref{cor:remainder-after-change-reduce-vector-modulo-alpha}, we have
    \begin{align*}
        f_{n}(\bm{a} + s\Phi_k\bm{e}_k) &\equiv f_{n}(\bm{a}) + \frac{\gamma}{\alpha^{k}}\cdot(\beta^{s\Phi_k} - 1) &&\pmod{\alpha}\\
        &\equiv f_{n}(\bm{a}) + \gamma\frac{\beta^{\Phi_k} - 1}{\alpha^{k}}\cdot\sum_{i=0}^{s-1}\beta^{i\Phi_k} &&\pmod{\alpha} \\
        % &\equiv f_{n}(\bm{a}) + \gamma\cdot(\nabla_1 + \alpha\mathcal{O}(1))\sum_{i=0}^{s-1}(\alpha^{k}(\nabla_1 + \alpha\mathcal{O}(1)) + 1)^{i} &&\pmod{\alpha} \\

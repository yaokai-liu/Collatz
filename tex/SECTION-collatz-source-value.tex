...
\begin{proposition}
    ...
    \end{align*}
\end{proposition}
\begin{proposition}\label{prop:simple-collatz-system-periodic}
    For simple Collatz system, we have
    \[ f_{n}(\bm{a} + s\Phi_k\bm{e}_k) \equiv f_{n}(\bm{a}).\pmod{\alpha} \]
\end{proposition}
\begin{proof}
    For simple Collatz system, we have $\Phi_k = \alpha^{k-1} \ord_{\alpha}\beta$.
    Then from \cref{cor:remainder-after-change-reduce-vector-modulo-alpha} and \cref{prop:power-order-mod}, we have
    \begin{align*}
        f_{n}(\bm{a} + \alpha\Phi_k\bm{e}_k) &\equiv f_{n}(\bm{a}) + \frac{\gamma}{\alpha^{k}}\cdot(\beta^{\alpha\Phi_k} - 1) &&\pmod{\alpha}\\
        &\equiv f_{n}(\bm{a}) + \frac{\gamma}{\alpha^{k}}\cdot(\beta^{\alpha^{k}\ord_{\alpha}\beta} - 1) &&\pmod{\alpha} \\
        &\equiv f_{n}(\bm{a}) + \gamma\cdot\alpha &&\pmod{\alpha}\\
        &\equiv f_{n}(\bm{a}). &&\pmod{\alpha}
    \end{align*}
\end{proof}

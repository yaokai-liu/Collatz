...
\end{proposition}
We denote $Z_{n}(N;\bm{v}) = \frac{\alpha^{n}\cdot N + \gamma\cdot\mathfrak{V}_{n}}{a\cdot \beta^{\mathcal{V}_{n}}}$, it will be very import in \cref{sec:homomorphic-densities-on-source-value-set}.
...
\begin{proposition}~\label{prop:collatz-character-1}
    $\val_{\beta} \chr a = \log_{\beta,\alpha}(a^{-1}\cdot \gamma)$. Thus if $\chr a < \infty$, we have $\chr a = \pow(\beta\mid \chr a) \cdot a$.
\end{proposition}
And according properties of the discrete logarithm, we have
\begin{proposition}~\label{prop:valchr-are-periodic}
    $\forall m\in\mathbb{N}$, there is $\val_{\beta}\chr a = \val_{\beta}\chr (a+m\alpha)$. Thus we have $\chr(a + m\alpha) = \chr a + m\alpha\cdot\pow(\beta\mid\chr a) = \pow(\beta\mid\chr a) \cdot (a + m\alpha)$.
\end{proposition}
\begin{remark}
    If $\chr a = \infty$, then we treat $\pow(\beta\mid \chr a) = \infty$. In fact, $m$ may be a negative number, provided that $a+m\alpha > 0$.
\end{remark}
...
\begin{theorem}~\label{thm:layers-of-reduce-space-and-source-value-set}
    ...
    \end{align*}
    Here the $\mathbb{S}$ is all primitive source defined in \cref{def:collatz-primitive-source}.
\end{theorem}
\begin{proof}
...
\end{remark}
\begin{corollary}~\label{cor:minial-of-n-layer}
    $\min\mathbb{F}_{n}(b) = f_{n}(\chr\bm{b};b)$. Here
    \[ \chr\bm{b} = (\val_{\beta}\chr b_{-n+1}, \cdots, \val_{\beta}\chr b_{-1}, \val_{\beta}\chr b). \]
\end{corollary}
...
\begin{proof}
    ...
    \end{align*}
\end{proof}
We need to introduce some symbols to simplify our expressions.
\begin{definition}
...
\begin{corollary}~\label{cor:source-value-set-equivlants-its-pre-chr-set}
    $\alpha\mathbb{F}_{n+1}(b) + \gamma = \mathbb{B}\otimes\chr\mathbb{F}_{n}(b)$. And the decomposition is uniqe.
\end{corollary}

% !TEX root = main.tex
\section{The basic formula of Collatz Sequences}\label{sec:the-basic-formula-of-collatz-sequences}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        =&\ a_{k+1} \cdot \beta^{\val_{\beta}(a;k+1)} \cdot \beta^{\mathcal{A}_{k}} \\
...
\end{corollary}
\begin{conjecture}
    $\exists c(\beta) \ge 0$, $\mu\{a \in\mathbb{Z}_{\beta} \mid \lim_{k\to\infty}\frac{\mathcal{A}_k}{k} \ge c \}  = 0$.
\end{conjecture}
...
But these propositions above provide a basic way to determine whether a number can be reduced, especially since we can easily filter out many initial numbers that cannot. For $(\alpha, \beta, \gamma) = (3, 2, 1)$, we have
\begin{corollary}
    $\forall a \in \mathbb{N} - 2\mathbb{N}$, $a\in[1]$, and $1 \le \frac{\mathcal{A}_k}{k} \le 2$ thus $\lim_{k\to\infty}\mathcal{A}_k = \infty$.
\end{corollary}
...
For example, $a=13$ has a 4-tempus cycle under $\alpha=3$, $\beta=5$, $\gamma=1$, that $\val_{5}(13;1) = 1$, $\val_{5}(13;2) = 2$, $\val_{5}(13;3) = 0$ and $\val_{5}(13;4) = 0$.
Obviously, $\val_{5}(13;i)$ are not same.

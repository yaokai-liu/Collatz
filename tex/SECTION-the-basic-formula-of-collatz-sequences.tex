...
\begin{itemize}
    \item[$\Longrightarrow$)]
        If $\lim_{k\to\infty} \mathcal{A}_k = \infty$, then $\forall k \in \mathbb{N}$, $\exists n_{k} \in \mathbb{N}_{\ge1}$, $\val_{\beta}(a;k + n_{k}) > 0$,
        that means $a_{k} \in [\val_{\beta}(a;k + n_{k}) | n_{k}] \subset [1]$.
    \item[$\Longleftarrow$)] 

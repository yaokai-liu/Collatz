...
\begin{proof}
    ...
    Here $\delta_2(p) = \fac_2(\beta + 1) - 1$ if $p = 2$ else $0$.
...
\end{proposition}
Actually we can derive $\mathbb{A}_{n+1}(b)$ from $\mathbb{A}_{n}(b)$.
...
\end{align*}
\end{proposition}
And From \cref{thm:vary-rule} and \cref{thm:the-periodic-rule-of-reduce-sequence}, we have
\begin{proposition}~\label{prop:layers-of-reduce-space-and-source-value-set}
    \begin{align*}
        \mathbb{V}_{n+1}(b) &= \left\{ (v, \bm{v}) \mid 2^{v}\cdot f \equiv 1\pmod{3}, v\in\mathbb{N}, f \in \mathbb{F}_{n}(b), \bm{v} \in \mathbb{V}_{n}(b) \right\} \\
        &= \bigcup_{f\in \mathbb{F}_{n}(b) - \mathbb{S}} \left\{ (2k + s, \bm{v}) \mid  k\in \mathbb{N}, \bm{v} \in \mathbb{V}_{n}(b) \right\}\\
        &= \left\{ \left.\sum_{i=1}^{n}(2k_i\cdot3^{i-1} + s_i)\bm{e}_{i} \right\vert  \forall k_i\in \mathbb{N} \right\},\\
...
We can observe that as the dimension $n$ increases, $\mathbb{V}_{n}(b)$ will become increasingly "sparse" because the terminal components between two integers will increase in multiples of $2 \cdot 3^{n}$. Actually, that's because mnay number has been put lower dimension space, and the primitive source also limit the dimension's extension.

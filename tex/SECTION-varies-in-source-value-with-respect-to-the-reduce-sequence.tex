...
\begin{theorem}[The Periodic Rule of Reduce Sequence]~\label{thm:single-integral-trans}
...
\end{proof}
\cref{thm:single-integral-trans} represents the changes $s$ that preserve integers, which do not depend on the original reduced sequence $\bm{v}$, but only on the position $k$.

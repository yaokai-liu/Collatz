% !TEX root = main.tex
\section{Back Trace of Collatz Sequence}\label{sec:back-trace-of-collatz-sequence}
\cref{theorem:any-presequence} informs us that we can also trace back Collatz sequences within $\mathbb{Q}$.
However, under the conditions of \cref{theorem:any-presequence}, we are able to perform this backtracking only subject to finite reduction constraints.
To maintain consistency with \cref{sec:a-kind-of-derivation-on-groupoid}, we must extend the scope of summation to infinity. Consequently, we redefine the summation symbol to incorporate the following directional property:
\begin{definition}

% !TEX root = main.tex
\section{Collatz Character and Collatz Core}\label{sec:collatz-character-and-collatz-core}

Although the periodicity of reduce vectors constitutes a significant result, we cannot stop there. As we learned in the previous section, not all reduce vectors lead to positive integers; indeed, many reduce vectors eventually resolve into fractions as the depth of the backtracking increases. Consequently, we are more interested in determining the conditions under which a number can be traced back to an integer, as well as the distribution of the resulting values.

To deal with this, we have to introduce two operators.
\begin{definition}\label{def:collatz-character-and-collatz-core}
Given $a \in\widehat{\mathbb{N}}_\beta$, define
\[ \chr a = \min\{v\in\mathbb{N}: \alpha\mid(\beta^{v}a-\gamma) \text{ and } \beta\nmid(\beta^{v}a-\gamma)/\alpha \} \]
called the Collatz character of $a$. If the set is empty, we say $a$ is a primitive source and define $\chr a = \infty$.
If $a$ is not primitive source, we define
\[ \cor a = \beta^{\chr a}a \]
as the source core of $a$.
Given $A \subset \widehat{\mathbb{N}}_\beta$, we denote
\begin{align*}
    \chr A &= \{ \chr a \mid a\in A - \mathbb{S}\}, \\
    \cor A &= \{ \cor a \mid a\in A - \mathbb{S}\}.
\end{align*}
\end{definition}
\begin{definition}
We define
\[\mathbb{S}(A) = A\cup\bigcup_{a\in A}\{ b\in \widehat{\mathbb{N}}_{\beta} \mid b\rightsquigarrow a \}\]
as all numbers that will fall into to $A$.
When $A = \{a\}$, that's $A$ contains only one number, we also denote it as $\mathbb{S}(a)$.
\end{definition}
\begin{proposition}
    If $a$ is reversible modulo $\alpha$, then $\chr a = \log_{\beta, \alpha}(a^{-1}\cdot \gamma)$.
\end{proposition}
\begin{proposition}
    $\cor a\equiv \gamma \pmod{\alpha}$.
\end{proposition}
And according properties of the discrete logarithm, we have
\begin{proposition}\label{prop:chr-le-ord}
    If $\chr a < \infty$, $\chr a \le \ord_{\alpha}(\beta)$.
\end{proposition}
\begin{theorem}\label{thm:collatz-character-finite-generated}
    $\chr a <\infty \Longleftrightarrow a\fmod \alpha \in [\gamma (\beta; \alpha)]/\alpha$.
\end{theorem}
\begin{proof}
    If $\chr a <\infty$, we can consider $\cor a$. Let $v = \ord_{\alpha}\beta - \chr a$,  we have
    \[ \cor a = \beta^{\chr a}a \equiv\beta^{\ord_{\alpha}\beta - v}a \equiv \beta^{-v}a \equiv\gamma. \pmod{\alpha} \]
    Thus $a\equiv \gamma\cdot\beta^{v} \pmod{\alpha}$ and $v\le \ord_{\alpha}\beta$. Thus $a\in[\gamma (\beta; \alpha)]/\alpha$.

    If $a\in[\gamma (\beta; \alpha)]/\alpha$, then $\exists v\le \ord_{\alpha}\beta$, such that $a\equiv \gamma\cdot\beta^{v} \pmod{\alpha}$. Then let $u = \ord_{\alpha}\beta - v \le \ord_{\alpha}\beta$, we have $\beta^{u}a\equiv\gamma \pmod{\alpha}$, which equivalents to $\alpha \mid (\beta^{u}a-\gamma)$. If $u > 0$, then $\beta\nmid(\beta^{u}a-\gamma)$, since $\gcd(\beta, \alpha) = 1$, thus we have $\beta\nmid (\beta^{u}a-\gamma)/\alpha$. If $\beta\mid(\beta^{u}a-\gamma)$, since $\gcd(\beta, \gamma) = 1$, we have $u = 0$. Thus $a-\gamma \equiv 0 \pmod{\alpha}$. Then we have $\beta\nmid (\beta^{\ord_{\alpha}\beta}a-\gamma) \equiv a-\gamma \equiv 0 \pmod{\alpha}$. Anyway, $\chr a\le \ord_{\alpha}\beta < \infty$.
\end{proof}
\begin{corollary}\label{cor:chr-are-periodic}
    $\chr a = \chr b \Longleftrightarrow b \equiv a \pmod{\alpha}$.
\end{corollary}
\begin{definition}
    Define $\mathbb{S}(a)_{i} = \{ b\rightsquigarrow a \mid \chr b = i \}$, $\mathbb{S}_{i} = \{ a\in\widehat{\mathbb{N}}_{\beta} \mid \chr a = i \}$.
\end{definition}
\begin{proposition}
    If denote $\overline{\Gamma} = [\gamma (\beta; \alpha)]/\alpha \cup\{\infty\}$, then
    $\mathbb{S}(a) = \bigcup_{i\in\overline{\Gamma}}\mathbb{S}(a)_{i}$ and $\widehat{\mathbb{N}}_{\beta} = \bigcup_{i\in\overline{\Gamma}}\mathbb{S}_{i}$.
\end{proposition}
Actually, $\mathbb{S}_{\infty}$ is exactly the set of all primitive sources. And we have
\begin{proposition}
    If $\alpha \mid \gamma$, then $\Phi_1 = 1$, and then $\chr b = \left\{\begin{aligned}
        &1, &&\alpha\mid b, \\
        &\infty, &&\alpha\nmid b.
    \end{aligned}\right.$\newline
    And thus $\mathbb{S}_{\infty} = \{b \in \widehat{\mathbb{N}}_\beta: \alpha\nmid b\}$.
    But if $\alpha \nmid \gamma$, then $\forall b\in\alpha\widehat{\mathbb{N}}_\beta$, $b \in \mathbb{S}_{\infty}$.
\end{proposition}

This proposition states that whether $\alpha \mid \gamma$ or $\alpha \nmid \gamma$ but $b \in \alpha\widehat{\mathbb{N}}_\beta$, these are all trivial cases. Thus when $\alpha \nmid \gamma$, given $b \in \mathbb{S}_{\infty}$. Or it is not reversible modulo $\alpha$, or
\[\Log_{\beta, \alpha}(b^{-1} \cdot \gamma) = \varnothing. \]
And $b$ is not reversible modulo $\alpha$ equivalents $\gcd(b, \alpha) > 1$. Hence we have
\begin{definition}
    Suppose $\alpha \nmid \gamma$. If $\gcd(b, \alpha) > 1$, we say they are normal primitive source.
    Others we call them abnormal primitive source.
    All normal primitive source are denoted as 
    \[\hat{\mathbb{S}}_{\infty} = \{b \in \widehat{\mathbb{N}}_{\beta} \mid \gcd(b, \alpha) > 1\} = \bigcup_{p\in\Omega(\mathbb{P}\mid\alpha)} p\widehat{\mathbb{N}}_{{\beta}}, \]
    all abnormal primitive source are denoted as
    \[ \tilde{\mathbb{S}}_{\infty} = \{b \in \widehat{\mathbb{N}}_{\beta} \mid \gcd(b, \alpha) = 1 \text{ and } \Log(b^{-1}\cdot\gamma) = \varnothing\}. \]
\end{definition}
\begin{proposition}
    $\mathbb{S}_{\infty} = \tilde{\mathbb{S}}_{\infty} \cup \hat{\mathbb{S}}_{\infty}$.
\end{proposition}
\begin{proposition}\label{prop:primitive-source-of-simple-collatz-system}
    If $\alpha\in\mathbb{P}$ and $\alpha\nmid\gamma$, then $\tilde{\mathbb{S}}_{\infty} = \varnothing$ and $\hat{\mathbb{S}}_{\infty} = \alpha\mathbb{N}_{\ge 1}$.
\end{proposition}

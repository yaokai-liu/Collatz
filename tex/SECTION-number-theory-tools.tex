...
\end{proposition}
\begin{definition}
    Define $\raf_{a}b = \prod_{p \in \Pi(\mathbb{P}\mid a)}p^{\fac_{p}b}$ as the filtered radical power of $b$ with respect to $a$. It is also called the $a$-filtered radical power of $b$ or the radical $a$-part of $b$. Specially, we define $\raf_{1}b = 1$.
    And we define $\ram_{a}b = \frac{b}{\raf_{a}b}$ called the remainder part of $b$ with respect to $a$.
\end{definition}
...
\begin{proposition}~\label{prop:raf-valuation-monotonicity}
    $\fac_{p}b_1 \le \fac_{p}b_2 \Longleftrightarrow \raf_{p}b_1 \mid \raf_{p}b_2$.
\end{proposition}
...
\end{proposition}
\begin{proposition}
      $\log \raf_{a}b = \sum_{p\in\mathbb{P}}^{p \mid a} \fac_{p}b \log p$
\end{proposition}

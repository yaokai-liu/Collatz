...
Let's define the symbol $\val_{2}(a;k+1) = \fac_{2}(3a_{k}+1)$ as count of factor $2$ in $3a_{k}+1$, where $a$ is the initial number given, and $k$ runs in $\mathbb{N}$.
...
\[ a_k \cdot 2^{\sum_{j=0}^{k-1}\val_{2}(a;j+1)} = a \cdot 3 ^ k + \sum_{i = 0}^{k - 1}2^{\sum_{j=0}^{i-1}\val_{2}(a;j+1)}\cdot3^{k-i-1}.  \]
With the definition $\mathcal{A}_{k} = \sum_{j=0}^{k-1}\val_{2}(a;j+1)$, it would be
...
Notice that $a$ is a positive finite number but $2^{\mathcal{A}_k}$ increase to infinite, that means $\sum_{i = 0}^{k - 1}2^{\mathcal{A}_i}\cdot3^{-i}$ should be a finite odd times of $3^{-1}$ under the module, which convergences to $1/3$ in $\mathbb{Z}_2$.
...
At first we define two auxity notation:
...
\begin{definition}[Collatz Sequence and Collatz Reduce Sequence]~\label{definition:collatz-sequence}
    ...
    For number $a \in \widehat{\mathbb{N}}_{{\beta}}$, define the sequence $\left\{a_{_k}\right\}_{k = 0}^{\infty} \subset \widehat{\mathbb{N}}_{{\beta}}$ named Collatz sequence and $\{\val_{\beta}(a;k)\}_{k=0}^{\infty}\subset\mathbb{N}$ named Collatz reduce sequence with
    \begin{align*}
        a_0 &=a, \\
        a_{k+1} \cdot \beta^{\val_{\beta}(a;k + 1)} &= \alpha\cdot a_{k} + \gamma.
    \end{align*}
    Of course, $\val_{\beta}(a;k + 1) = \fac_\beta(\alpha\cdot a_{k} + \gamma)$.
    Actually, $\val_{\beta}(a;k+1)$ is exactly the $\beta$-adic valuation of $\alpha\cdot a_{k} + \gamma$.
\end{definition}

Given a number $a \in \widehat{\mathbb{N}}_{{\beta}}$, we can derive a Collatz sequence and Collatz reduce sequence. For the Collatz reduce sequence, we can define its accumulation, and we call it Collatz accumulation.
...
\end{align*}
\end{proof}
Actually, we can formally define the Collatz complement only from \cref{proposition:collatz-complement-basic-prop1} and \cref{proposition:collatz-complement-basic-prop2}. More essentially, the two propositions are just the basic rules of a quasi $(\sigma, \tau)$-derivation of \cref{sec:a-kind-of-derivation-on-groupoid}, which in other words, the Collatz complement is a kind of $(\sigma, \tau)$-derivation on groupoid (\cref{def:quasi-derivation-on-groupoid}).

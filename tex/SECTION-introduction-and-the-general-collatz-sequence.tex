...
Let's define the symbol $\val(a;k+1) = \fac_{2}(3a_{k}+1)$ as count of factor $2$ in $3a_{k}+1$, where $a$ is the initial number given, and $k$ runs in $\mathbb{N}$.
With this we can redefine the sequence with
\[ a_{k + 1} = \frac{3a_{k}+1}{2^{\val(a;k+1)}} = \ram_{2}(3a_{k}+1), \]
...
\begin{definition}[Collatz Sequence and Collatz Reduce Sequence]~\label{definition:collatz-sequence}
    ...
    \begin{align*}
        &a_0=a, \\
        &\val(a;k) = 0, \\
        &a_{k+1} = \frac{\alpha\cdot a_{k} + \gamma}{\beta^{\val(a;k + 1)}} = \ram_{\beta}(\alpha\cdot a_{k} + \gamma), \\
        &\val(a;k+1) = \fac_{\beta}(\alpha\cdot a_k + \gamma).
    \end{align*}
\end{definition}
Actually, $\val(a;k+1)$ is exactly the $\beta$-adic valuation of $\alpha\cdot a_{k} + \gamma$.
...
\begin{proof}
    ...
    Second, if $a_{k} = a'_{i_k}$. Suppose $s_{k+1} = \fac_{\beta}(3a'_{i_k} + 1)$. We have
    \[ s_{k+1} = \fac_{\beta}(3a_k + 1) = \val(a;k+1). \]
    For $a'_{i_k}$, we have
    \begin{align*}

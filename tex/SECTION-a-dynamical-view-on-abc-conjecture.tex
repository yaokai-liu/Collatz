...
\begin{proposition}\label{prop:det-factorization-matrix-is-diff}
	Let $\mathcal{S} = \mathcal{P} - \mathcal{Q}$, we have $\det \bm{M} = (-1)^{n}\cdot\mathcal{S}$.
\end{proposition}
\cref{prop:det-factorization-matrix-is-diff} reveals the connection between $\mathcal{S}$ and the factors of $\mathcal{P}$ and $\mathcal{Q}$ in the equation $\mathcal{P} + \mathcal{S} = \mathcal{Q}$; this bears a close relationship to the ABC conjecture. Actually, with this theorem, we have
...
\end{conjecture}
\begin{proposition}
	Let $\bm{\gamma} = \bm{M}\bm{r}$, there is or $\exists k < n$ such that $q_k \le p_k + 1$, or $\norm{\bm{\gamma}}_{\infty} > 1$.
\end{proposition}
\begin{proof}
	If not, i.e. $\forall k < n$, $q_k \ge p_k + 2$ and $\norm{\bm{\gamma}}_{\infty} = 1$. Then we have $\norm{\bm{\gamma}}_{\infty} = 1$, then $\abs{\gamma_k} = 1$. From $p_k \cdot r_k = q_k\cdot r_{k+1} + \gamma_k$, we have $p_k \cdot \abs{r_k} = q_k \cdot \abs{r_{k+1}} - \abs{\gamma_k} = q_k \cdot \abs{r_{k+1}} - 1$. Since $q_k \ge p_k + 2$, then $p_k \cdot \abs{r_k} \ge p_k \cdot \abs{r_{k+1}} + 2 \abs{r_{k+1}} - \abs{\gamma_k} \ge p_k \cdot \abs{r_{k+1}} + 1$. Thus we have $\abs{r_k} \ge \abs{r_{k+1}}$. Thus we have $\abs{r_k} \ge \abs{r_{k+n}} = \abs{r_k}$, which is paradoxical. 
\end{proof}
\begin{proposition}\label{prop:power-diff-to-dynamical-system}
	Suppose $\alpha, \beta \in \mathbb{N}_{\ge2}$ are coprime. For given $C \in \mathbb{Z}$, the equation $\beta^{B} - \alpha^{A} = C$ possesses an integer solution $A, B$ if and only if $\exists n \in \mathbb{N}$ and sequences $\bm{a}, \bm{b} \in \mathbb{N}^{n}$ such that when $p_k = \beta^{b_k}$, $q_k = \alpha^{a_k}$, there is $\abs{\det\bm{M}} = C$.
\end{proposition}
\begin{proposition}\label{prop:collatz-cycle-system} 
	For $p_k$ be the power of number $2$ and $q_k = 3$, the classic Collatz has a non-trivial cycle equivalents to $\forall n \ge 1$, $\forall \bm{r} \in \mathbb{Z}^{n} - \{(1, 1, \cdots, 1)\}$ there is $\bm{M}\bm{r} \ne (1, 1, \cdots, 1)$.
\end{proposition}
% \cref{prop:power-diff-to-dynamical-system} posits a perspective: given a constant $C$, we can derive solutions $A$ and $B$ by searching for partitions of the exponents.
% Indeed, as established by the analysis in the preceding sections, every equation of the form presented in \cref{prop:power-diff-to-dynamical-system} corresponds to a specific "top branch" of a certain $t$-periodic power-difference dynamical system.
...
Since $\bm{M}$ is an integer coefficient matrix, consider it's Simith normal form, unfortunately, for this kind of matrix, the Simith normal form will be $\diag(1, 1, \cdots, (-1)^{n}\mathcal{S})$, which can't get anly new factorization information from it. Thus we have
\begin{definition}
	Given $\bm{M}\in \mathbb{Z}^{n\times n}$ that $\det\bm{M} \ne 0$. Define factorization form is $\mathcal{L}(M) = \diag(m_0, m_1, \cdots, m_{n-1})$ such that $\forall p\in\mathbb{P}$, there is
	...
	\end{aligned}\right. \]
	Of course, $\mathcal{L}(M)$ be a kind of Simith normal form. Since $\mathcal{S} = \abs{\det\bm{M}}$, we also denote it as $\mathcal{L}(\mathcal{S})$.
\end{definition}
\begin{proposition}
	$\rad\mathcal{S} = \abs{m_{n-1}}$ and $\mathcal{L}(\mathcal{S}^{\sharp}) = \begin{pmatrix}
	0 & 0 \\
	\bm{I} & 0 \\
...
\begin{definition}\label{def:der-rad-rings}
	Suppose $R_1 = R(\mathbb{K})/\mathbb{K}$ is an integral domain modulo its algebrical closed $\mathbb{K}$ characteristic $0$. We require $\rad: R_1 \to R_1$ and $\partial, \overline{\partial}: R_1 \to R_1$, they satisfy:
	\begin{enumerate}
...
\end{align*}
Of course, here ($+$) also means separator instead of ring addition. That means even $s + t  = f + g$, we may have no $\overline{\delta}(s + t) = \overline{\delta}(f + g)$. But we still require $\overline{\partial}(s + t) = \overline{\partial}(f + g)$, and in this formula, ($+$) is the ring addition.
Similar with Mason-Stothers theorem, we have
\begin{proposition}\label{prop:fgh-mid-lcm-rad}
	If $f + g = h$, then we have
...
\end{align*}
Here we do not require $\mathcal{K}(f, g)$ be an element of $R_1$, but it should be the natural localization domain of $R_1$.
For polynomials, the $\mathcal{K}(f, g) = \frac{f'}{f} - \frac{g'}{g}$. However, for natural numbers, we don't know yet what the $\mathcal{K}(f, g)$ and $\mathcal{M}(f, g)$ represent for.

...
We may wonder the maximum value of $m$ which depends on $a_k$, what the $a_k$ depends on $a$ and $k$.
...
\end{definition}
With these we have
\begin{definition}\label{definition:collatz-sequence}
    Given numbers $\alpha, \beta\in \mathbb{N}_{\ge2}$ and $\gamma \in\mathbb{N}_{\ge 1}$ that $\gcd(\beta, \alpha\gamma) = 1$.\newline
    For number $a \in \widehat{\mathbb{N}}_{{\beta}}$, define the sequence $\left\{a_{k}\right\}_{k = 0}^{\infty} \subset \widehat{\mathbb{N}}_{{\beta}}$ named Collatz sequence and $\{\val(a;k)\}$ named Collatz reduce sequence with
    \begin{align*}
        &a_0=a, \\
        &\val(a;k) = 0, \\
    ...
    \end{align*}
\end{definition}
In fact, this definition is precisely the generalization of the Syracuse function\cite{wikipediaCollatzConjecture} to arbitrary arguments. 
...
\end{definition}
Normally, we will always lowercase letters like $u$, $v$, $w$, $\cdots$ to represent elements of some sequences or vectors.
\begin{proposition}
...
\end{proof}
\begin{proposition}
    If $\exists t > 0$ that $a_{t} = a_0 = a$, then $\forall k > 0$ and $s \le t$, there are
...
\begin{proof}
    ...
    Hence we have
    \[ a_{k+1} = (3a_k + 1) / 2^{\val(a;k+1)} = (3a'_{i_k} + 1) / 2^{s_{k+1}} = a'_{i_k+s_k} = a'_{i_{k+1}}. \]
    Sum up, we proved this proposition.
\end{proof}
...
With $(\alpha, \beta, \gamma) = (3, 2, 1)$,
for any odd number $a \in \widehat{\mathbb{N}}_{2}$, it's Collatz sequence will finally reach to $1$.
\end{conjecture}

This article involves many terms like $\textstyle \sum_{i = 0}^{k - 1}\beta^{\mathcal{A}_i}\cdot \alpha^{k-i-1}$. We first define some notations to represent them:
...
For Collatz complements, we have two basic propositions:
...
More essentially, the two propositions are exactly the basic rules of a quasi $(\sigma, \tau)$-derivation of \cref{sec:a-kind-of-derivation-on-groupoid}, which in other words, the Collatz complement is a kind of $(\sigma, \tau)$-derivation on groupoid (\cref{def:quasi-derivation-on-groupoid}). Besides, we have
...
\end{proposition}
\begin{proposition}\label{prop:complement-coprime-with-alpha-and-beta}
    Suppose $m\ne n$, we have $\mathfrak{V}_n^{m} \equiv \beta^{\mathcal{A}_{n-1}^{m}} \pmod{\alpha}$ and hence $\gcd(\alpha, \mathfrak{V}_n^{m}) = 1$. And if $v_{1} \ne 0$, then we also have $\mathfrak{V}_n^{m} \equiv \alpha^{n-m-1} \pmod{\beta}$ and hence $\gcd(\alpha, \mathfrak{V}_n^{m}) = 1$.
\end{proposition}

...
Because when we start iteratively to apply the definition of $a_{k-1}$ into the definition of $a_{k}$, we can find
\[ a_k \cdot 2^{\sum_{j=0}^{k-1}\val(a;j+1)} = a \cdot 3 ^ k + \sum_{i = 0}^{k - 1}2^{\sum_{j=0}^{i-1}\val(a;j+1)}\cdot3^{k-i-1}.  \]
With the definition $\mathcal{A}_{k} = \sum_{j=0}^{k-1}\val(a;j+1)$, it would be
...
\begin{definition}[Collatz Sequence and Collatz Reduce Sequence]~\label{definition:collatz-sequence}\nomenclature{$a_k$}{The $k$-th item of Collatz sequence of $a$, \cref{definition:collatz-sequence}}\nomenclature{$\val(a;k)$}{The $k$-th item of Collatz reduce sequence of $a$, \cref{definition:collatz-sequence}}
    Given three arguments $\alpha, \beta, \gamma \in \mathbb{N}_{\ge1}$ that $\gcd(\alpha, \beta) = 1$, $\gcd(\beta, \gamma) = 1$, and $\alpha \ne 1, \beta\ne 1$.\newline
    ...
    \end{align*}
\end{definition}
Actually, $\val(a;k+1)$ is exactly the $\beta$-adic valuation of $\alpha\cdot a_{k} + \gamma$. In common, we will always lower latter like $a$, $b$, $c$, $\cdots$ to present numbers of a field.
...
\end{definition}
Normally, we will always lower latter like $u$, $v$, $w$, $\cdots$ to present elements of some sequences or vectors.
\begin{proposition}
...
\end{proposition}
\begin{proposition}
    If $\forall v_i \in \{v_i\}: v_i \ge 1$, then $\beta\nmid \mathfrak{A}_n^{m}$ and $\alpha \nmid \mathfrak{A}_n^{m}$.
\end{proposition}

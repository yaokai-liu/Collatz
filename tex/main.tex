...
\end{aligned}\]
\end{proof}
Actually, we can formally define the Collatz complement only from \cref{proposition:collatz-complement-basic-prop1} and \cref{proposition:collatz-complement-basic-prop2}. More essentially, the two propositions are just the basic rules of convolution form of \cref{sec:two-abstract-forms}, which in other words, the Collatz complement is a kind of convolution form (\cref{def:convolution-form}).
...
\end{corollary}
\begin{conjecture}
    There's an upper bound of $\frac{\mathcal{A}_k}{k}$ when $k\to\infty$, and it doesn't depend on $\alpha$.
\end{conjecture}
...
\end{aligned}\right. \]
    Here $m, n \in \mathbb{Z}$, thus $\mathfrak{A}_{n}^{m} \in \mathbb{Z}$ and $\mathfrak{a}_{n}^{m}\in\mathbb{Q}$.
...
Immediately, we have
\begin{proposition}
    $f_{n}(\bm{a}) \overset{n}{\rightsquigarrow} b$.
\end{proposition}
...
\end{proposition}
\begin{proposition}
    $f_{n}(\bm{a}) = \frac{\beta^{\Delta_{1}}}{\alpha}\left(\frac{\beta^{\Delta_{2}}}{\alpha}\left(\cdots \left(\frac{\beta^{\Delta_{n}}}{\alpha}\cdot b - \frac{\gamma}{\alpha}\right)\cdots\right)- \frac{\gamma}{\alpha}\right) - \frac{\gamma}{\alpha}$.
\end{proposition}
\begin{proposition}
...
Given $\bm{s} \in \mathbb{Z}^{n}$, view it as a $n$-length Collatz sequence, thus we can using $\mathcal{S}$, $\mathfrak{S}$ and $\mathfrak{s}$. Then we have
\[ f_{n}(\bm{a} + \bm{s}) = \beta^{\mathcal{S}_{n}}\cdot f_{n}(\bm{a}) + \frac{\gamma}{\alpha^{n}}\cdot\sum_{k=0}^{n-1}\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}\cdot\alpha^{n-k-1}\cdot(\beta^{\mathcal{S}_{n}^{k}} - 1). \]
\end{theorem}
\begin{proof}
    Notice that
    ...
    where $F_{\bm{s}}(\mathfrak{a})_{n}$ is the reduced Collatz complement of $\bm{a} + \bm{s}$ that
    \[ F_{\bm{s}}(\mathfrak{a})_{n} = \sum_{k=0}^{n-1}\frac{\beta^{\mathcal{A}_k + \mathcal{S}_k}}{\alpha^{k}}. \]
    Then
    \[\begin{aligned}
        & f_{n}(\bm{a} + \bm{s}) - \beta^{\mathcal{S}_{n}}\cdot f_{n}(\bm{a}) \\
        =& \frac{\gamma}{\alpha}\cdot\left(\beta^{\mathcal{S}_{n}}\cdot\mathfrak{a}_{n} - F_{\bm{s}}(\mathfrak{a})_{n} \right) \\
...
\end{proof}
From now we denote
\[ \Gamma_n(b) = \frac{\gamma}{\alpha^{n}}\cdot\sum_{k=0}^{n-1}\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}\cdot\alpha^{n-k-1}\cdot(\beta^{\mathcal{S}_{n}^{k}} - 1). \]
Then this proposition can be written as $f_{n}(\bm{a} + \bm{s}) = \beta^{\mathcal{S}_{n}}\cdot f_{n}(\bm{a}) + \Gamma_n(b)$. Additionally, if no ambiguous, we abbreviate $\Gamma_n(b)$ to $\Gamma$.
Actually we have
\begin{corollary}
    $\Gamma = \frac{\gamma}{\alpha}\cdot\beta^{\mathcal{S}_n}\cdot\mathfrak{a}_n - \frac{\gamma}{\alpha}\cdot\mathfrak{a}'_{n}$. Here $\mathfrak{a}'$ is for $\bm{a} + \bm{s}$.
\end{corollary}

And immediately, we have
\begin{corollary}~\label{corollary:integral-trans}
    $\Gamma \in \mathbb{N}$ equivalents to 
    \[ \gamma \cdot \sum_{k=0}^{n-1}\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}\cdot\alpha^{n-k-1}\cdot(\beta^{\mathcal{S}_{n}^{k}} - 1) \equiv 0 \pmod{\alpha^{n}}. \]
\end{corollary}
\begin{remark}
    If $\mathcal{S}_{n}^{k} < 0$, that means the times of $\beta$ is over gather to $\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}$, instead of the inverse under the module of $\alpha^{n}$.
\end{remark}
If $\alpha^{n} \mid\gamma$, then this congruence equation is trivial.
Otherwise, the analysis of this congruence equation would be very difficult.
However, if there is another $\bm{s}$ with only one non-zero weight, i.e. $\bm{a} + \bm{s}$ degenerates into $F_{s;k}(\bm{a})$, then the analysis becomes easier.
\begin{theorem}~\label{thm:single-integral-trans}
    If $f_{n}(\bm{a}) \in \mathbb{N}$, then
    \[f_{n}(F_{s;k}(\bm{a})) \in \mathbb{N}\Longleftrightarrow \left.\frac{\alpha^{k}}{\raf_{\alpha}(\gamma\cdot2^{\delta})}\cdot\Psi_{\alpha}(\beta)\right\vert s, \]
...
\end{proof}
\cref{thm:single-integral-trans} represents the changes $s$ that preserve integers, which do not depend on the original reduced sequence $\bm{a}$, but only on the position $k$.
...
That is, if $f_{n}(\bm{a})\overset{n}{\rightsquigarrow} b$ and $f_{n}(\bm{a} + \bm{s}) \overset{n}{\rightsquigarrow} b$, then starting from a certain bit $m$ and $m + \mathcal{S}_n$, the $\beta$-carrying terms of these two numbers satisfy a certain relationship. Mathematically, that's
\begin{theorem}~\label{thm:high-order-bits-sync}
    Suppose $f_{n}(\bm{a}) = \sum_{k=0}^{\infty}x_k\beta^{k}$ and $f_{n}(\bm{a} + \bm{s}) = \sum_{k=0}^{\infty}x'_k\beta^{k}$. If $\Gamma \in \mathbb{N}$, then $\exists m \ge 0$, such that
    \[ \sum_{k=0}^{\infty}x'_{k + m}\beta^{k} = \sum_{k=0}^{\infty}x_{k + m - \mathcal{S}_n}\beta^{k} + \delta, \]
    where the $\delta\in\{0, 1\}$. Vice versa.
\end{theorem}
\begin{proof}
    Since $\Gamma \in \mathbb{N}$, that means $\deg_{\beta}(\Gamma)<\infty$. Let $m = \max\{\deg_{\beta}(\Gamma), \mathcal{S}_n\} + 1$, then
    \[ \beta^{m - 1}\le\beta^{\mathcal{S}_n}\sum_{k=0}^{m - \mathcal{S}_n - 1}x_k\beta^{k} + \Gamma \le \beta^{m}. \]
    And from \cref{thm:vary-rule}, we have
    \[ \sum_{k=0}^{\infty}x'_k\beta^{k} = \beta^{\mathcal{S}_n}\sum_{k=0}^{m - \mathcal{S}_n - 1}x_k\beta^{k} + \Gamma + \beta^{\mathcal{S}_n}\sum_{k=m}^{\infty}x_{k}\beta^{k}. \]
    Let's right shift both the series by $m$, then we have
    \[ \sum_{k=m}^{\infty}x'_k\beta^{k - m} = \sum_{k=m - \mathcal{S}_n}^{\infty}x_{k}\beta^{k-m + \mathcal{S}_{n}} + \delta. \]
    Adjusting the subscript yields
    \[ \sum_{k=0}^{\infty}x'_{k + m}\beta^{k} = \sum_{k=0}^{\infty}x_{k + m - \mathcal{S}_n}\beta^{k} + \delta. \]
    On the other hand, if there is $m\ge 0$ make the series equation holds, then $\forall k \ge m$ the series equation also holds. So suppose $m \ge \mathcal{S}_n + 1$. We can see from the previous derivation that
    \[ \Gamma = \sum_{k=0}^{m-1}x'_k\beta^{k} - \beta^{\mathcal{S}_n}\sum_{k=0}^{m - \mathcal{S}_n - 1}x_k\beta^{k} + \delta\cdot\beta^{m}, \]
    thus $\deg_{\beta}(\Gamma) \le m \le\infty$, thus $\Gamma \in \mathbb{N}$.
\end{proof}
Notice that we don't require the $f_{n}(\bm{a})$ and $f_{n}(\bm{a} + \bm{s})$ in $\mathbb{N}$, that means they can be any $\beta$-adic integer.
\begin{remark}
    If $f_{n}(\bm{a}) \not\in \mathbb{Z}_{<0}$, then $\exists m > \max\{\deg_{\beta}(\Gamma), \mathcal{S}_n\} + 1$, such that \cref{thm:high-order-bits-sync} holds and $\delta = 0$. But if $f_{n}(\bm{a}) \in \mathbb{Z}_{<0}$ and $\Gamma \ge \left\vert \beta^{\mathcal{S}_n}\cdot f_{n}(\bm{a}) \right\vert_{\mathrm{abs}}$, then $\forall m$ large enough, there is only $\delta = 1$, i.e. $f_{n}(\bm{a} + \bm{s}) \in \mathbb{N}$.
\end{remark}
Even though, we can't determinate integer source values only depends on $\Gamma\in\mathbb{N}$, because there are evidences present that even if $\Gamma\not\in\mathbb{N}$, we can derive an integer source value.

\begin{definition}
Given a target set $B$ Denote that
\[ \begin{aligned}
    \mathbb{A}_n(B) &= \bigcup_{b\in B}\left\{ \bm{s} \in \mathbb{Z}^{n} \mid b\cdot\beta^{\mathcal{S}_n} + \Gamma_n(b) \in \mathbb{N} \right\},\\
    \mathbb{F}_n(B) &= \bigcup_{b\in B}\left\{ f_n(\bm{a} + \bm{s}) \mid \bm{s} \in \mathbb{A}_n(b) \right\}.
\end{aligned} \]
Specially, we define $\mathbb{A}_0(B) = \varnothing$ and $\mathbb{F}_0(B) = B$. When $B = \{b\}$, that's $B$ contains only one number, we also denote as  $\mathbb{A}_n(b)$ and $\mathbb{F}_n(b)$.
\end{definition}
\begin{conjecture}
    For $(\alpha, \beta, \gamma) = (3,2,1)$, and $\forall n\ge 1$, $\bm{a} = (2, 2, 2, \cdots, 2)$, we have $\bigcup_{n = 0}^{\infty}\mathbb{F}_n(1) =\mathbb{N} - 2\mathbb{N}$.
\end{conjecture}
Actually we can derive $\mathbb{A}_{n+1}(b)$ from $\mathbb{A}_{n}(b)$. But since we are calculating the source value, the positive direction of the vector's dimensions is from the source value to the target value. Therefore, if we want to obtain an $(n+1)$-dimensional vector from an $n$-dimensional vector, we should add the extra components to the beginning of the vector, rather than to the end. Of course, the condition of $\Gamma_n$ should be satisfied anyway.
For $(\alpha, \beta, \gamma) = (3,2,1)$, we have
\begin{proposition}
\[ \begin{aligned}
    \mathbb{A}_{n+1}(b) &= \left\{ (\Delta, \bm{s}) \mid 2^{\Delta}\cdot f \equiv 1\pmod{2}, f \in \mathbb{F}_{n}(b), \bm{s} \in \mathbb{A}_{n}(b) \right\},\\
    \mathbb{F}_{n+1}(b) &= \left\{ (2^{\Delta}\cdot f  - 1)/3 \mid 2^{\Delta}\cdot f \equiv 1\pmod{2}, f \in \mathbb{F}_{n}(b) \right\}.
\end{aligned} \]
\end{proposition}
\begin{proposition}
    \[\mathbb{F}_{n+1}(b) = \bigcup_{f\in \mathbb{F}_{n}(1) - \mathbb{S}} \left\{f_0 = (2^{\Delta}\cdot f  - 1)/3, f_{k+1} = 4f_{k} + 1 \mid k\in\mathbb{N}\right\},\]
    Where the $\mathbb{S}$ is all primitive source defined as \cref{collatz-primitive-source}, and $\Delta$ is the min value such that $2^{\Delta}\cdot f \equiv 1\pmod{2}$.
\end{proposition}
Actually, each $k$ represents add-on times to the first component $\Delta$ by $2$.
% Now we given the transform rule of $\Gamma_{n}$.


% Since the congruence of \cref{corollary:integral-trans} equation can be convert a series of prime number's congruence.
% That's $\forall p\in\mathbb{P}$ that $p \mid \alpha$, such that
% \[\gamma \cdot \sum_{k=0}^{n-1}\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}\cdot p^{n-k-1}\cdot(\beta^{\mathcal{S}_{n}^{k}} - 1) \cdot \rem_{p}(\alpha^{n-k-1}) \equiv 0 \pmod{p^{n\cdot\fac_{p}(\alpha)}}, \]
% we can first consider $\alpha\in \mathbb{P}$. For $p \in \mathbb{P}$, we have
% \[ \gamma \cdot \sum_{k=0}^{n-1}\beta^{\mathcal{A}_{k} + \mathcal{S}_{k}}\cdot p^{n-k-1}\cdot(\beta^{\mathcal{S}_{n}^{k}} - 1) \equiv 0 \pmod{p^{n}}. \]
...
\begin{definition}[Collatz primitive source]~\label{collatz-primitive-source}
    For $b\in \mathbb{N}-\beta\mathbb{N}$, if there is a sequence $\bm{a}\in \mathbb{N}^{n}$ such that $f_{n}(\bm{a})$ is not reversible modulo $\alpha$, or
    \[\Log_{\beta, \alpha}(f_{n}(\bm{a})^{-1} \cdot \gamma) = \varnothing, \]
    then we say $\bm{a}$ is a $n$-step Collatz primitive source vector of $b$ and $f_{n}(a)$ is a $n$-step Collatz primitive source of $b$.
    ...
    \[\mathbb{S}_c = \{b \in \mathbb{N}-\beta\mathbb{N} \mid \gcd(b, \alpha) = 1 \text{ and } \Log(b^{-1}\cdot\gamma) = \varnothing\}, \]
    all primitive source denotes as
    \[ \mathbb{S} = \{b \in \mathbb{N}-\beta\mathbb{N} \mid \gcd(b, \alpha) > 1\}\cup \mathbb{S}_c. \]
\end{definition}
If $f_n(\bm{a}) \in \mathbb{S}$, then we have $\forall k \in \mathbb{N}$, there is
...
Immediately, we have
\begin{proposition}
    If $(\alpha, \beta, \gamma) = (3,2,1)$, then $\mathbb{S}_c = \varnothing$.
\end{proposition}
\begin{proposition}
...
For $(\alpha, \beta, \gamma) = (3,2,1)$, we have
\begin{proposition}
    Suppose $f_{n}(\bm{s}) \in \mathbb{S}$, then $\left\{f_{n}(\bm{s} + 6\bm{e}_1) \mid k\in\mathbb{N}\right\} \subset \mathbb{S}$.
\end{proposition}
\appendix
...
\begin{definition}
Given two numbers $\frac{p_1}{q_1}, \frac{p_2}{q_2} \in \mathbb{Q}$ and they are irreducible, define $\frac{p_1}{q_1}\mid \frac{p_2}{q_2}$ as $\exists t \in \mathbb{Z}_{\ne0}$ such that $t\cdot \frac{p_1}{q_1} = p_2$. If not, we say $\frac{p_1}{q_1}\nmid \frac{p_2}{q_2}$. Of course we stipulate that $\forall \frac{p_1}{q_1} \in \mathbb{Q}$, $\frac{p_1}{q_1}\mid 0$.
\end{definition}
\begin{proposition}
...
\[- \Log_{a, b}(x) = \ord_a(b)\mathbb{Z} - \log_{a, b}(x).\]
\end{definition}
We call $\log_{a, b}(x)$ the principal modular logarithm of $x$, and $\Log_{a, b}(x)$ the modular logarithms of $x$, and define $\log_{a, b}(1) = 0$ and $\Log_{a, b}(1) = \ord_{b}(a)\mathbb{Z}$.
...
From \cref{sec:the-basic-fomula-of-collatz-sequences}
...
\end{proposition}
\begin{definition}[Convolution Form]~\label{def:convolution-form}

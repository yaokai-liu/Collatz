...
Back to the \cref{prop:integral-trans}.
Since the congruence equation can be convert a series of prime number's congruence.
...
\begin{definition}[Collatz primitive source]~\label{collatz-primitive-source}
    ...
    \[\mathbb{S} = \{b \in \mathbb{N}-\beta\mathbb{N} \mid \Log(b^{-1}\cdot\gamma) = \varnothing\}, \]
    and all non-trivial primitive source vector of $b$ denoted as
    \[ \mathbb{V}_{b}^{n} = \{\bm{a} \in \mathbb{N}^{n} \mid \Log(f_{n}(a)^{-1}\cdot\gamma) = \varnothing\}. \]
\end{definition}
...
Now let's consider the $0$-step primitive source. 
\begin{proposition}

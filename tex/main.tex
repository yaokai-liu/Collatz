...
\maketitle
\begin{abstract}
    This paper explores various properties of generalized Collatz sequences of the form $a_{k+1}\cdot\beta^{\val(a;k+1)} = \alpha\cdot a_{k} + \gamma$, and extends this form to the rational number field $\mathbb{Q}$ and finds it's a kind of $(\sigma, \tau)$-derivation of groupoid. We construct a hierarchical model for the source value set in $\mathbb{N}$. We prove that the logarithmic density of the latent algebraic structure in these sets can be preserved, thus providing a comprehensive algebraic landscape for discrete iterative processes. We analyze a certain sub-type of general Collatz system, they are similar to the $3n+1$ problem, and give a classification of its source value sets. In particular, we provide a positive results for the original conjecture. Finally, this paper also explores the power difference problem from the perspective of dynamical systems.
...
\input{SECTION-extend-codomain-for-collatz-sequence}
...
\input{SECTION-a-dynamical-system-view-in-power-difference-problem}

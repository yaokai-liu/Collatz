...
Notice that $a$ is a positive finity number but $2^{\mathcal{A}_k}$ increase to infinity, that means $\sum_{i = 0}^{k - 1}2^{\mathcal{A}_i}\cdot3^{-i}$ should be a finity odd times of $3^{-1}$ under the modula.
...
\end{theorem}
But not all $b$ is reversible modulo $\alpha$, and not all $a^{-1} \cdot \gamma$ is a remainder of a power of $\beta$.
So we have
...
\begin{proof}
    ...
    \end{aligned} \]
    That means $p_{i}^{m \cdot \fac_{p_i}(\beta)} \left\vert\ \sum_{i=0}^{P - 1}\alpha^{-i} \right. $.
    From the Chinese Remainder Theorem, we have $\beta^{m} \left\vert\ \sum_{i=0}^{P - 1}\alpha^{-i} \right. $.
...
\section{On $\alpha=3$, $\beta=2$, $\gamma=1$}\label{sec:on-alpha3-beta2-gamma1}
Now suppose $\alpha=3$, $\beta=2$ and $\gamma=1$.
...
\end{lemma}
\begin{lemma}
    If $b\cdot 2^{\Delta} = 3a + 1$, let $\Delta = 2n + \log_{2, 3}(b)$ and $m = \floor{b/3}$, then
...
\end{corollary}
To proof the conjecture, we define some functions.
\begin{definition}
    For $\bm{a} = (\Delta_{1}, \Delta_{2}, \cdots, \Delta_{n}) \in \mathbb{N}_{\ge1}^{n}$, with the notation of $\mathcal{A}_0 = 0$ and $\mathcal{A}_{k} = \sum_{i=1}^{k}\Delta_{i}$, define function $f_{n} : \mathbb{N}_{\ge1}^{n} \to \mathbb{Q}$ as
...
\begin{proposition}
    If $f_{n}(\bm{a}) \in \mathbb{N}$, then $f_{n}(\bm{a}) \overset{n}{\rightsquigarrow} 1$.
\end{proposition}
% \begin{proposition}
...
\begin{proposition}
    If $m > 0$, then we have 
    \[g_{s,m;n}(\bm{a}) \overset{n}{\rightsquigarrow} s \cdot 2^{m} \cdot 3^{n} + 1 \overset{(m - 1)/2}{\rightsquigarrow} \left\{\begin{aligned}
...
\end{lemma}
We can see that these $\mathfrak{U}_{k+1}^{k}$ play an important role on consisting of $\mathfrak{U}_{i}^{j}$.
...
\begin{theorem}
    Given two cycle multiply form $\mathcal{A}$ and $\mathcal{B}$ with period $t$. then $\forall \mathfrak{U} \in \mathrm{CCon}_{t}(\mathcal{A}, \mathcal{B})$, we have $\mathfrak{U}$ is a cycle form with peroid $t$.
\end{theorem}
\begin{proof}
    \[ \begin{aligned}

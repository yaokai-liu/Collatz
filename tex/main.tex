...
=&\ \beta^{t}\cdot f_{n}(F_{s;k}(\bm{a})) + \frac{\gamma}{\alpha}\cdot(\beta^{t} - 1)\cdot F_{s;k}(\mathfrak{a})_{l}^{0} \\
=&\ \beta^{s}\cdot\beta^{t}\cdot\mathfrak{a}_{k}^{0} - \frac{\gamma}{\alpha}\cdot\mathfrak{a}_{k}^{0}+\frac{\gamma}{\alpha}\cdot\beta^{t}\cdot\beta^{s}\cdot\mathfrak{a}_{l}^{k} - \frac{\gamma}{\alpha}\cdot\beta^{s}\cdot\mathfrak{a}_{l}^{k} \\
=&\ \beta^{t+s}\cdot f_{n}(\bm{a}) + \frac{\gamma}{\alpha}\cdot\mathfrak{a}_{k}^{0}\cdot(\beta^{t+s} - 1) + \frac{\gamma}{\alpha}\cdot\mathfrak{a}_{l}^{k}\cdot\beta^{s}\cdot(\beta^t - 1).
\end{aligned}\]
\end{proposition}
...
\begin{theorem}~\label{thm:integral-trans}
    ...
    where the auxiliary operator $\Psi_{\alpha}(\beta)$ is defined as \[
        \Psi_{\alpha}(\beta) = \underset{p\mid\alpha}{\lcm}\{\rem_{p}\ord_{p}(\beta)\}\cdot\prod_{p\in\mathbb{P}}^{p \mid \alpha}\frac{\raf_{p}\ord_{p}(\beta)}{\raf_{p}(\beta^{\ord_{p}(\beta)} - 1)}.
    \]
\end{theorem}
\begin{proof}
    From \cref{prop:vary-rule}, we have
    \[ f_{n}(F_{s;k}(\bm{a})) = \beta^{s}\cdot f_{n}(\bm{a}) + \frac{\gamma}{\alpha}\cdot\mathfrak{a}_{k}\cdot(\beta^{s} - 1). \]
    ...
    From \cref{prop:raf-keeps-multiply}, we have
    ...
    \[ \left. \frac{p^{k\cdot\fac_{p}(\alpha)}}{\raf_{p}(\gamma\cdot2^{\delta})}\cdot\frac{\raf_{p}\ord_{p}(\beta)}{\raf_{p}(\beta^{\ord_{p}(\beta)} - 1)} \right\vert s. \]
    Besides, we'v assumed $\ord_{p}(\beta) \mid s$. Since $\raf_{p}\ord_{p}(\beta) \mid s$, thus we need the $\rem_{p}\ord_{p}(\beta) \mid s$. Aggregate these all $p$ conditions, it follows
    \[ \underset{p\mid \alpha}{\lcm}\{ \rem_{p}\ord_{p}(\beta) \} \mid s. \]
...
Furthermore, this conclusion can be easily generalized to the study of the periodic properties of linear or fractional dynamical systems in other algebraic fields, simply by replacing the prime factors with ideals and the lcm with the least common ideal, while the form remains essentially unchanged. However, such generalizations are beyond the scope of this paper, which will continue to focus on Collatz sequences.
...
where the inverse is about $\alpha^{n+1}$.

We can find that the destribution of Collatz primitive source deeply relay on $\beta^{k}$ modula $\alpha$.
...
\begin{definition}
    ...
    Define $\rem_{a}(b) = \frac{b}{\raf_a{b}}$ is called the remained part of $b$ with respect to $a$.
\end{definition}
\begin{proposition}
    $\raf_{a}(b) \mid b$.
\end{proposition}
...
\end{proposition}
\begin{proposition}~\label{prop:raf-keeps-multiply}
...
\end{proposition}
\begin{proposition}
    If $a_1 \mid a_2$, then $\raf_{a_1}(b) \mid \raf_{a_2}(b)$.
\end{proposition}
\begin{proposition}
    If $\fac_{p}(b_1) \le \fac_{p}(b_2)$, then $\raf_{p}(b_1) \mid \raf_{p}(b_2)$. Vice versa.
\end{proposition}
\begin{proposition}
    If $\rad(a_1) = \rad(a_2)$, then $\raf_{a_1}(b) = \raf_{a_2}(b)$.
\end{proposition}
\begin{proposition}
...
\end{proposition}
\begin{theorem}
    If $p\in\mathbb{P}$ that $p > 2$, then $\raf_{p}(b^{n} - 1) = \raf_{p}(b - 1) \cdot \raf_{p}(n)$.
\end{theorem}

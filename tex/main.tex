...
\begin{theorem}
    Given two multiply form $\mathcal{A}$ and $\mathcal{B}$.
    If there is $T \in I$ such that $\mathcal{A}_{i+T}^{j+T} = \mathcal{A}_{i}^{j}$ and $\mathcal{B}_{i+T}^{j+T} = \mathcal{B}_{i}^{j}$, then $\forall \mathfrak{U} \in \mathrm{ICon}(\mathcal{A}, \mathcal{B})$, then $\mathfrak{U}_{i+T}^{j+T} = \mathfrak{U}_{i}^{j}$.
\end{theorem}
\begin{proof}
    \[ \begin{aligned}

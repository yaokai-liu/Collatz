...
We now return to the subscript component of our notation. If these subscripts were restricted solely to the set of natural numbers $\mathbb{N}$, they would be insufficient to form a groupoid. Consequently, we must define their "inverse" forms. Fortunately, Theorem \cref{theorem:any-presequence} informs us that, within the domain of the rational numbers $\mathbb{Q}$, we are indeed able to trace Collatz sequences in reverse.
However, under the conditions stipulated by Theorem \cref{theorem:any-presequence}, this reverse-tracing operation is constrained by a finite limit on the number of reduction steps.
To ensure consistency with the content of Section \cref{sec:a-kind-of-derivation-on-groupoid}, we must extend the scope of summation to infinity. Accordingly, we have redefined the summation symbol to incorporate the directional characteristics illustrated below:
\begin{definition}

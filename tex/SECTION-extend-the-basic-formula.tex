% !TEX root = main.tex
\section{Extend The Basic Formula}\label{sec:extend-the-basic-formula}

A better thought is extending our view from $\mathbb{N}$ to $\mathbb{Q}$.
Keeping $\alpha, \beta, \gamma \in \mathbb{N}$, instead of restricting the initial number $a \in \mathbb{N}$, we can let $a$ be any number of $\widehat{\mathbb{Q}}_{{\beta}}$.
...
\begin{theorem}~\label{theorem:any-cycle}
    Given $\{v_{i}\}_{i=1}^{n}$ that $n < \infty$, if $\forall i>j\ge1, \beta \nmid \mathfrak{A}_i^{j}$, then there exists only one Collatz sequence in $\widehat{\mathbb{Q}}_{{\beta}}$ is a cycle that $\val(a;k) = v_{k\fmod n}$.
\end{theorem}
\begin{proof}%[Proof of \cref{theorem:any-cycle}]
    Let's just using the notation $\mathcal{A}_k$ and $\mathfrak{A}_k$ as if $\val(a;k) = v_{k\fmod n}$.\newline
    Then $\forall k = 1, 2, \cdots, n$, we have $\mathcal{A}_{k + n}^{k} = \mathcal{A}_n$.
    Let
    \[ a_k = \frac{\gamma\cdot\mathfrak{A}_{n+k}^{k}}{\beta^{\mathcal{A}_n} - \alpha^n} \in \widehat{\mathbb{Q}}_{{\beta}}. \]
...
\begin{theorem}~\label{theorem:any-presequence}
    Given $\{v_{i}\}_{i=1}^{n}$ that $n < \infty$, and $b \in \widehat{\mathbb{Q}}_\beta[b]$.
    If $\forall i>j\ge1, \beta \nmid \mathfrak{A}_i^{j}$, then there is only one Collatz sequence in $\widehat{\mathbb{Q}}_{{\beta}}[b]$ that $\val(a;i) = v_{i}$ and $a_n = b$.
\end{theorem}
\begin{proof}
    Also using the notation $\mathcal{A}_k$, $\mathfrak{A}_k$ and $\mathfrak{a}_k$ as if $\val(a;i) = v_{i}$.
    Let
    \[ a_k = \frac{b \cdot \beta^{\mathcal{A}_n^{k}} - \gamma\cdot\mathfrak{A}_n^{k}}{\alpha^{n-k}} = b \cdot \frac{\beta^{\mathcal{A}_n^{k}}}{\alpha^{n-k}}  - \frac{\gamma}{\alpha}\cdot\alpha^{k}\cdot\mathfrak{a}_n^{k} \in \widehat{\mathbb{Q}}_{{\beta}}[b] \]
...
\end{proof}
\begin{corollary}
    Given $a = \frac{   p}{q}\in \widehat{\mathbb{Q}}_{{\beta}}$. If $\exists s\in \mathbb{N}$, such that $q = \alpha^{s}$, then $a_{s} \in \mathbb{N}$ by natual reduce sequence.
\end{corollary}
...
\begin{theorem}
    If Collatz sequence of $a$ will fall into a cycle, then $a \in \widehat{\mathbb{Q}}_{{\beta}}$.
\end{theorem}
...
\begin{conjecture}
    Given $\{v_{i}\}_{i=1}^{\infty}$ has no cycle, then there is a Collatz sequence that $\val(a;i) = v_{i}$, it has a subsequence $\{a_{k_i}\}_{i=0}^{\infty}$ converges to an $\beta$-adic irrational number or $0$.
\end{conjecture}
...
% We have to note that the remainder of this section bears little direct relevance to our main line of reasoning; it has been included solely to satisfy the algebraic requirements outlined in \cref{sec:a-kind-of-derivation-on-groupoid}. Consequently, readers who are not interested in the underlying algebraic phenomena may safely skip this portion and proceed directly to the next section.

% Now we return to the subscripts used in our notation.
% If these subscripts were restricted exclusively to the set of natural numbers $\mathbb{N}$, they would be insufficient to constitute a groupoid. Therefore, we must define their "inverse" forms. Fortunately, Theorem \cref{theorem:any-presequence} informs us that, within the domain of rational numbers $\mathbb{Q}$, it is indeed possible to trace Collatz sequences in reverse.
% However, under the conditions stipulated by Theorem \cref{theorem:any-presequence}, this reverse-tracing operation is constrained by the finite nature of the reduction steps involved. Now,
% in order to endow the aforementioned supplementary material with "directional algebraic closure," we must modify the traditional summation operator into one that is sensitive to direction. Accordingly, we have redefined the summation symbol to incorporate the directional characteristics illustrated below:
% \begin{definition}

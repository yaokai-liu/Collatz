...
% 	Now let's consider the relationship between $H$ and $M$. Since $F_{n}(b)$ is a infinite set, thus given $ M' > M$, if $\forall n > H(M)$, there is $\{M < F_{n}(b) \le M' \} = \varnothing$ that $\card\{\mathbb{F}_{n}(b)\le M'\} = \card\{\mathbb{F}_{n}(b)\le M\}$, then $H(M') = H(M)$. If $\exists n\ge H(M)$ such that $\{M < F_{n}(b) \le M' \} \ne \varnothing$, then we have $H(M') > H(M)$. Sum up, we have $H(M') \ge H(M)$, thus $H$ is increasing according to $M$.
% \end{proof}
\begin{corollary}~\label{cor:source-value-set-finite-decomposition}
	...
	\[ \card\{\mathbb{F}(b) \le N\} = \card\left\{\bigcup_{n=0}^{H-1}\mathbb{F}_{n}(b)\le N\right\} = \sum_{n=0}^{H-1}\card\{\mathbb{F}_{n}(b) \le N\}. \]
\end{corollary}
\cref{thm:finite-layers-le-M} Tell us that we can by explore how $H$ increase with $M$ and the density of $\mathbb{F}(b)$ to predic how many top branches in this system. 

\begin{theorem}
	Given $a, b\in\widehat{\mathbb{N}}_{\beta} - \mathbb{S}$, we have $\lim_{N\to\infty}\frac{\card\{\mathbb{F}_{1}(a) \le N\}}{\card\{\mathbb{F}_{1}(b) \le N\}} = 1$.
\end{theorem}
% \begin{proof}
% 	\begin{align*}
...
% \end{proof}
\begin{corollary}
	$\forall a, b \in \widehat{\mathbb{N}}_\beta - \mathbb{S}$, we have $\lim_{N\to\infty}\frac{\card\{\mathbb{F}_{n}(a) \le N\}}{\card\{\mathbb{F}_{n}(b) \le N\}} > 0$.
\end{corollary}
...
In other words, if we can find a such homomorphic density and prove the density of $\mathbb{F}(1)$ is almost $1/2$, we then prove \cref{conj:source-value-of-1-cover-all-odd-numbers}, which derives to the classic Collatz conjecture.

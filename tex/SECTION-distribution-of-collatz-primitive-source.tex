...
\begin{theorem}~\label{thm:primitive-source-are-periodic}
    Let $b \in \mathbb{S}$. $\forall m \in \mathbb{Z}$, if $b + m\alpha > 0$, then $\rel_{\beta}(b + m\alpha) \in \mathbb{S}$.
\end{theorem}
\begin{proof}
    Trivial for $\alpha \mid \gamma$. Suppose $\alpha \nmid \gamma$.

    For those $b \in \hat{\mathbb{S}}$, we have $\gcd(b, \alpha) > 1$. Let $d = \gcd(b, \alpha)$, then we have $\gcd(d, \beta) = 1$. Thus $d\mid \rel_{\beta}(b + m\alpha)$ means $\rel_{\beta}(b + m\alpha) \in \hat{\mathbb{S}}$.

    For those $b \in \tilde{\mathbb{S}}$, we have $\gcd(b + m\alpha,\ \alpha) = 1$. If $\rel_{\beta}(b + m\alpha) \not\in \mathbb{S}$, then $\exists k \ge 1$, such that $\beta^{k}\equiv \rel_{\beta}(b + m\alpha)^{-1} \cdot \gamma \pmod{a}$. Thus we have
    \vspace{-0.5em}
    \begin{align*}
        \beta^{k - \val_{\beta}(b + m\alpha)} &= \beta^{k}\cdot \pow(\beta\mid b + m\alpha)^{-1} \\
        &\equiv \pow(\beta\mid b + m\alpha) \cdot \rel_{\beta}(b + m\alpha)^{-1} \cdot \gamma &&\pmod{a} \\
        &\equiv (b + m\alpha)^{-1} \cdot \gamma &&\pmod{\alpha} \\
        &\equiv b^{-1} \cdot \gamma &&\pmod{\alpha},
    \end{align*}
    which implies that $b \not\in \mathbb{S}$, conflicts with the hypothesis. Therefore, it indicates that there must be $\rel_{\beta}(b + m\alpha)\in \tilde{\mathbb{S}}$.
\end{proof}
\begin{remark}

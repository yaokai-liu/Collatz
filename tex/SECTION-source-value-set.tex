...
\begin{proposition}~\label{prop:source-set-include}
    If $a\rightsquigarrow b$, $\mathbb{S}(a) \subset \mathbb{S}(b)$.
\end{proposition}
...
\begin{proposition}~\label{prop:gcd-1-derive-not-all-primitive}
    Suppose $\gcd(\alpha, \nabla_1) = 1$. Then $\forall b\not\in\mathbb{S}_{\infty}$, $\forall n\ge 1$, we have $\mathbb{S}_{n}(b)/\alpha\mathbb{Z} = \mathbb{Z}/\alpha\mathbb{Z}$.
\end{proposition}
% \begin{proof}
%     We prove $n=1$ at first.\newline
%     Suppose $a \in\mathbb{S}_{1}(b) \cap \mathbb{S}$, and its reduce vector is $\bm{a}$.
%     From \cref{lem:modulo-alpha-of-f_1}, $\forall k\in\mathbb{Z}$ that $f_{1}(\bm{a} + k\Phi_1\bm{e}_1) > 0$, there is $f_{1}(\bm{a} + k\Phi_1\bm{e}_1) \equiv k\nabla_1 + f_{1}(\bm{a}) \pmod{\alpha}$ runs all remainders modulo $\alpha$. Thus $\mathbb{S}_{1}(b)/\alpha\mathbb{Z} = \mathbb{Z}/\alpha\mathbb{Z}$.\newline
%     Now consider any layer $n$. Since any $n$-step source value is just a $1$-step source value of a $n-1$-step source value. Thus they follow the conclusion of $n=1$. Thus we proved $\mathbb{S}_{n}(b)/\alpha\mathbb{Z} = \mathbb{Z}/\alpha\mathbb{Z}$.
% \end{proof}
...
%     Thus from \cref{prop:chr-are-periodic}, $f_{1}(\bm{a} + k\Phi_1\bm{e}_1) \in \mathbb{S}$. Notice that $f_{1}(\bm{a} + k\Phi_1\bm{e}_1)$ runs all $\mathbb{S}_{1}(b)$, thus $\mathbb{S}_{1}(b) \subset \mathbb{S}$.
% \end{proof}
...
\begin{theorem}~\label{thm:layers-of-reduce-space-and-source-value-set}
    \begin{align*}
        ...
        &= \bigcup_{a\in \mathbb{S}_{n+1}(b) - \mathbb{S}_{\infty}} \left\{ \left.\wchr\bm{a} + \sum_{i=1}^{n+1}k_i\Phi_i\bm{e}_{i} \right\vert  \forall k_i\in \mathbb{N} \right\}.
    \end{align*}
\end{theorem}
% \begin{proof}
%     Trivial for $\mathbb{S}_{n}(b) - \mathbb{S}_{\infty} = \varnothing$.\newline
%     Suppose $\mathbb{S}_{n}(b) - \mathbb{S}_{\infty} \ne \varnothing$, which means $\forall a\in \mathbb{S}_{n}(b) - \mathbb{S}_{\infty}$, we have $\chr a < \infty$. Then $A_0 = \frac{\cor a - \gamma}{\alpha} \in\widehat{\mathbb{N}}_{\beta}$, and hence $A_0 = f_{1}((\chr a);b)\in \mathbb{S}_{1}(a)\subset \mathbb{S}_{n+1}(b)$. From \cref{cor:single-vary-rule}, we have
%     \begin{align*}
...
%     On the other hand, $\forall a\in\mathbb{S}_{n+1}(b)$, from \cref{prop:rewritten-collatz-rule}, we have
%     \[\beta^{\simp\val(a;1)\Phi_1}\cor a_1 = \alpha a + \gamma. \]
...
%     And $a_1\in\mathbb{S}_{n}(b)$ by definition, thus $a = \frac{1}{\alpha}\cdot\beta^{\simp\val(a;1)\Phi_1}\cdot\cor a_1 - \frac{\gamma}{\alpha}$ in the set of the right side.\newline
...
% \end{proof}
\begin{remark}
    Notice that in $\wchr\bm{a} + \sum_{i=1}^{n+1}k_i\Phi_i\bm{e}_{i}$, the start number $a$ is in $\mathbb{S}_{n+1}(b)$ instead of $\mathbb{S}_{n}(b)$, thus $a_i\in\mathbb{S}_{n-i+1}(b)$.
\end{remark}
\begin{corollary}~\label{cor:source-value-set-equivlants-its-pre-ker-set}
    $\alpha\mathbb{S}_{n+1}(b) + \gamma = \mathbb{B}\otimes\cor\mathbb{S}_{n}(b)$. And the decomposition is uniqe.
\end{corollary}
...
That's because they are still based on $\mathbb{S}_{n}(b)$, and therefore still require recursively representing all the first $n$ $\mathbb{S}$s to represent the $(n+1)$-th.
...
% 	If $b \not\rightsquigarrow b$. Since $b\not\in\mathbb{S}$, from \cref{prop:gcd-1-derive-not-all-primitive}, we have $\mathbb{S}_{N}(b) \not\subset \mathbb{S}$. That means $\mathbb{S}_{N + 1}(b) \ne \varnothing$.
...
\begin{example}~\label{coro:layers-of-reduce-space-and-source-value-set-on-3-2-1}
    ...
    \begin{align*}
        ...
        \mathbb{V}_{n+1}(b) &= \bigcup_{a\in \mathbb{S}_{n+1}(b) - \mathbb{S}_{\infty}}\left\{ \left.\wchr\bm{a} + \sum_{i=1}^{n+1}2k_i\cdot3^{i-1}\bm{e}_{i} \right\vert  \forall k_i\in \mathbb{N} \right\}.
    \end{align*}
\end{example}
With this, we have

...
\end{proof}
\begin{proposition}
    If $b \not\rightsquigarrow b$, then $\forall r, s\in\mathbb{S}_{n}(b)$, we have $r \not\rightsquigarrow s \not\rightsquigarrow r$.
\end{proposition}
\begin{proposition}\label{prop:cycle-layer-included}
If $b\overset{t}{\rightsquigarrow} b$, then $\forall k\ge 0$, $b \in \mathbb{S}_{kt}(b)$, and hence $\mathbb{S}_{kt}(b)\subset \mathbb{S}_{(k+1)\cdot t}(b)$.
\end{proposition}
\begin{definition}
    Suppose $b\overset{t}{\rightsquigarrow} b$ and $t$ is the minimal step. Define $\overline{\mathbb{S}}_{0}(b) = \{ b_{k} \mid 0 \le k < t \}$ and $\forall n\ge 1$, $\overline{\mathbb{S}}_{n}(b) = \left(\bigcup_{k=0}^{t-1}\mathbb{S}_n(b_k)\right)$ called the $n$-th layer closure source value set of loop that contains $b$, we call $t$ is the length of the loop.
\end{definition}
Of course, for those $b$ that $t = 0$, we have $\overline{\mathbb{S}}_{n}(b) = \mathbb{S}_{n}(b)$.
\begin{proposition}
    Define $\Delta\overline{\mathbb{S}}_{n}(b) = \overline{\mathbb{S}}_{n}(b) - \overline{\mathbb{S}}_{n-1}(b)$. Given $k$ that $1 < k \le t$. $\forall s \in \Delta\overline{\mathbb{S}}_{n}(b)$, there is $b \not\rightsquigarrow s$, but $s \overset{1}{\rightsquigarrow} b_k \overset{t-k}{\rightsquigarrow} b$. Given $r, s\in \Delta\overline{\mathbb{S}}_{n}(b)$ that $r \ne s$, we have $r \not\rightsquigarrow s \not\rightsquigarrow r$. 
\end{proposition}
These $\Delta\overline{\mathbb{S}}_{n}(b)$ behave exactly like those source value of a non-loop target. In fact, we have $\Delta\overline{\mathbb{S}}_{n}(b) = \overline{\mathbb{S}}_{n}(b) = \mathbb{S}_{n}(b)$ if $b\not\rightsquigarrow b$.

From \cref{cor:remainder-after-change-reduce-vector-modulo-alpha}, we still have
\begin{proposition}
...
\begin{proposition}\label{prop:have-primitive-derive-all-primitive}
    Suppose $\gcd(\alpha, \gamma) = 1$ and $\alpha\mid\nabla_1$. If $\exists b\not\in\mathbb{S}_{\infty}$ such that $\mathbb{S}_{1}(b) \cap \mathbb{S}_{\infty} \ne \varnothing$, then $\mathbb{S}_{1}(b) \subset \mathbb{S}_{\infty}$.
\end{proposition}
\begin{proof}
    Suppose $f_{1}(\bm{a}) \in\mathbb{S}_{1}(b) \cap \mathbb{S}_{\infty}$.
    From \cref{cor:remainder-after-change-reduce-vector-modulo-alpha}, $\forall k\in\mathbb{Z}$ such that $f_{1}(\bm{a} + k\Phi_1\bm{e}_1) > 0$, we have
    \[ f_{1}(\bm{a} + k\gamma\Phi_1\bm{e}_1) \equiv k\nabla_1 + f_{1}(\bm{a}) \equiv f_{1}(\bm{a})  \pmod{\alpha} \]
    Thus from \cref{cor:chr-are-periodic}, $f_{1}(\bm{a} + k\Phi_1\bm{e}_1) \in \mathbb{S}_{\infty}$. Notice that $f_{1}(\bm{a} + k\Phi_1\bm{e}_1)$ runs all $\mathbb{S}_{1}(b)$, thus $\mathbb{S}_{1}(b) \subset \mathbb{S}_{\infty}$.
\end{proof}
...
\begin{theorem}\label{thm:layers-of-reduce-space-and-source-value-set}
    \begin{align*}
        \mathbb{S}_{n+1}(b) &= \bigcup_{a\in \mathbb{S}_{n}(b) - \mathbb{S}_{\infty}} \left\{\left. A_0 = \frac{\cor a  - \gamma}{\alpha}, A_{k+1} = \beta^{\Phi_1} A_{k} + \gamma\nabla_1 \right\vert k\in\mathbb{N}\right\} \\
...
We may wary about if there's only finite layers could be traced back, but
\begin{theorem}\label{thm:infinite-layers}
	Suppose $\gcd(\alpha, \gamma\nabla_1) = 1$. For $b\not\in\mathbb{S}_{\infty}$, $\forall N > 0$, there is $\mathbb{S}(b) - \bigcup_{n=0}^{N}\mathbb{S}_{n}(b) \ne \varnothing$.
\end{theorem}
\begin{proof}
	...
	If $b \rightsquigarrow b$. Suppose it's a $t$-period cycle, then $b \in\mathbb{S}_{t}(b)$. From \cref{thm:layers-of-reduce-space-and-source-value-set}, we can just pick $a = \beta^{\Phi_1}\cdot b + \nabla_1\in\mathbb{S}_{t}(b)$, we have $a \rightsquigarrow b \not\rightsquigarrow a$. From previous discussion, $\mathbb{S}(a)$ can not be covered within finity layers, thus the $\mathbb{S}(b) \supset \mathbb{S}(a)$ also can't be covered within finity layers.
\end{proof}

\cref{thm:infinite-layers} demonstrates that we can never resolve the Collatz conjecture by any method of case analysis by number's patterns, especially remainders modulo some specially numbers, even if we partition the problem into various infinite sets. It's because different layers are in defferent patterns. Though we can tell a lot of patterns, and for any given layer, we can also explore its pattern, that's not enough at all.
...
\begin{proposition}
    $S(k + 1; b) > S(k; b)$.
\end{proposition}
\begin{proposition}\label{prop:congrunce-transform}
    If $b = a + m\cdot \alpha$, then we have
    \[ S(k; b) = S(k; a) + m\cdot\beta^{\chr a + k\Phi_1}. \]
\end{proposition}
% \begin{proof}
%     \begin{align*}
%         S(k; b) &= \frac{1}{\alpha}\beta^{k\Phi_1}\cdot\cor b - \frac{\gamma}{\alpha} \\
%         &= \frac{1}{\alpha}\beta^{k\Phi_1}\cdot\beta^{\chr b} b - \frac{\gamma}{\alpha} \\
%         &= \frac{1}{\alpha}\beta^{k\Phi_1}\cdot\beta^{\chr a} (a + m\cdot \alpha) - \frac{\gamma}{\alpha} \\
%         &= \frac{1}{\alpha}\beta^{k\Phi_1}\cdot\beta^{\chr a} a - \frac{\gamma}{\alpha} +  m\beta^{k\Phi_1}\cdot\beta^{\chr a} \\
%         &= S(k; a) + m\cdot\beta^{\chr a + k\Phi_1}
%     \end{align*}
% \end{proof}
\begin{definition}

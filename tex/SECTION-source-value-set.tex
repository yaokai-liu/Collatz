...
\end{proof}
\cref{thm:infinite-layers} demonstrates that we can never resolve the Collatz conjecture by any method of case analysis by number's patterns, even if we partition the problem into various infinite sets. It's because different layers are in defferent patterns. Though we can tell a lot of patterns, and for any given layer, we can also explore its pattern, that's not enough at all.

...
\begin{proposition}\label{prop:det-factorization-matrix-is-diff}
	$\det\bm{M} = \mathcal{P} - \mathcal{Q}$. And $\det(\lambda\bm{I} - \bm{M}) = \sum_{i=0}^{n}\sigma_{i}^{n}\lambda^{n-i} + (-1)^{n+1}\mathcal{Q}$. Here
	\[ \sigma_{i}^{n} = \sum_{0\le j_1 < j_2 < \cdots < j_i \le n-1} p_{j_1}p_{j_2}\cdots p_{j_i}. \]
\end{proposition}
\begin{example}[ABC conjecture]\label{exa:rewitten-abc-conjecture}

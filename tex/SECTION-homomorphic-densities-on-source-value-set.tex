...
\begin{proposition}
	If $A\otimes B$ and $A \otimes C$ are both unique decomposed and $\mu'(B, C)$ or $\mu'(A\otimes B,  A\otimes C)$ exists, then we have
	$\mu'(A\otimes B,  A\otimes C) = \mu'(B, C)$.
\end{proposition}
% \begin{remark}
...
\begin{theorem}~\label{thm:proportional-of-density-of-source-value-set}
	...
	\[\mu'(\mathbb{F}(a),\mathbb{F}(b)) = \frac{\chr b}{\chr a}. \]
\end{theorem}
% \begin{lemma}
% 	$\forall n \ge 1$, we have
% 	\[ \mu'(\chr\mathbb{F}_{n}(b), \mathbb{F}_{n}(b)) = \frac{\beta^{\Phi_1} - 1}{\beta^{\alpha\Phi_1} - 1}\cdot\sum_{i=1}^{\alpha-1}\beta^{i\Phi_1-v(\alpha-i)}. \]
...
% 	From \cref{prop:source-set-dijoint-primitive-source}, the elements of $\mathbb{F}_{n}(b)$ are cycled and ergodic modulo $\alpha$. Thus $\forall i , j$ that $\alpha \ge i, j > 0$, we have
% 	\[ \beta^{i\Phi_1}\mu\widetilde{\mathbb{F}_{n}(b)}_{i} \approx \beta^{j\Phi_1}\mu\widetilde{\mathbb{F}_{n}(b)}_{j}. \]
% 	Let $x \approx \beta^{i\Phi_1}\mu\widetilde{\mathbb{F}_{n}(b)}_{i}$, we have
% 	\[\mu\mathbb{F}_{n}(b) \approx \sum_{i=1}^{\alpha}\beta^{-i\Phi_1}x = \frac{x}{\beta^{\alpha\Phi_1}}\cdot\frac{\beta^{\alpha\Phi_1} - 1}{\beta^{\Phi_1} - 1}.\]
% 	Thus $\mu\widetilde{\mathbb{F}_{n}(b)}_{i} \approx \frac{\beta^{\alpha\Phi_1}}{\beta^{i\Phi_1}}\frac{\beta^{\Phi_1} - 1}{\beta^{\alpha\Phi_1} - 1}\mu\mathbb{F}_{n}(b)$. And then we have
% 	\begin{align*}
% 		\mu\chr\mathbb{F}_{n}(b) &= \sum_{i=1}^{\alpha-1}\beta^{-v(i)}\mu\widetilde{\mathbb{F}_{n}(b)}_{i} \\
% 		&\approx \sum_{i=1}^{\alpha-1}\beta^{-v(i)}\frac{\beta^{\alpha\Phi_1}}{\beta^{i\Phi_1}}\frac{\beta^{\Phi_1} - 1}{\beta^{\alpha\Phi_1} - 1}\mu\mathbb{F}_{n}(b)\\
% 		&= \mu\mathbb{F}_{n}(b)\cdot\frac{\beta^{\Phi_1} - 1}{\beta^{\alpha\Phi_1} - 1}\cdot\sum_{i=1}^{\alpha-1}\beta^{i\Phi_1-v(\alpha-i)}.
% 	\end{align*}
% 	Thus we have
% 	\[\mu'(\chr\mathbb{F}_{n}(b), \mathbb{F}_{n}(b)) = \frac{\beta^{\Phi_1} - 1}{\beta^{\alpha\Phi_1} - 1}\cdot\sum_{i=1}^{\alpha-1}\beta^{i\Phi_1-v(\alpha-i)}.\]
% \end{proof}
\begin{lemma}~\label{lem:finite-layers-le-M}
	...
	$\forall M > 0$, $\exists H > M$ depends on $M$ such that $\forall n \ge H$, there is $\card\{ \mathbb{F}_{n}(b)\le M \} = 0$.
\end{lemma}
\begin{proof}
	Since $b\not\rightsquigarrow b$, thus $\forall i, j \ge 0$, $\mathbb{F}_{i}(b)\cap \mathbb{F}_{j}(b) = \varnothing$.
	We can construct a sequence of set that $A_{n} = \{x\in\mathbb{M} \mid x \le M\} - \bigcup_{k=0}^{n-1}\mathbb{F}_{k}(b)$. We have $A_{n+1} \subset A_{n}$. Thus $\{A_n\}_{n=1}^{\infty}$ has a minimal element. Suppose it's $A_m$. Thus let $ H = \min\{k\in\mathbb{M}\mid A_{k} = A_{m}\}$, $\forall n\ge H$, we have $\card\{\mathbb{F}_{n}(b)\le M\} = 0$.

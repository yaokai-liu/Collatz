...
\begin{proof}
	\begin{align*}
		...
		=&\lim_{N\to\infty}\frac{\card\{k\in\mathbb{N} \mid \frac{1}{\alpha}\beta^{k\Phi_1}\chr a - \frac{\gamma}{\alpha} \le N\}}{\card\{k\in\mathbb{N} \mid \frac{1}{\alpha}\beta^{k\Phi_1}\chr b - \frac{\gamma}{\alpha} \le N\}} \\
		=&\lim_{N\to\infty}\frac{\card\{k\in\mathbb{N} \mid k \le \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr a\}}{\card\{k\in\mathbb{N} \mid k \le \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr b\}} \\
		=&\lim_{N\to\infty}\frac{\lfloor\frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr a\rfloor}{\lfloor \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr b\rfloor} \\
...
\end{theorem}
\begin{lemma}
	If $\card\{\mathbb{F}_{n}(a) \le N\} > 0$, then $\frac{\alpha^{n} N + \gamma\cdot \chr\mathfrak{A}_{n}}{a\beta^{\chr\mathcal{A}_{n}}} \ge 1$.
\end{lemma}
\begin{proof}
	Trivial from \cref{prop:condition-of-source-value-le-N} and \cref{cor:minial-of-n-layer}.
\end{proof}
% \begin{proof}[Proof of \cref{thm:proportional-of-density-of-source-value-set}]
% 	Denote $\widetilde{\log}_{n}(N;\bm{x}) = \log_{\beta}\left(\frac{\alpha^{n}N + \mathfrak{X}_{n}}{x\beta^{\mathcal{X}_{n}}}\right)$, we have
% 	\begin{align*}
% 		&\mu'(\mathbb{F}_{n}(a), \mathbb{F}_{n}(b)) \\
% 		=&\lim_{N\to\infty}\frac{\card\{\mathbb{F}_{n+1}(a) \le N\}}{\card\{\mathbb{F}_{n+1}(b) \le N\}} \\
% 		=&\lim_{N\to\infty}\frac{\card\bigcup_{x\in\mathbb{F}_{n}(a) - \mathbb{S}}\{k\in\mathbb{N} \mid \frac{1}{\alpha}\beta^{k\Phi_1}\chr x - \frac{\gamma}{\alpha} \le N\}}{\card\bigcup_{y\in\mathbb{F}_{n}(b) - \mathbb{S}}\{k\in\mathbb{N} \mid \frac{1}{\alpha}\beta^{k\Phi_1}\chr y - \frac{\gamma}{\alpha} \le N\}} \\
% 		=&\lim_{N\to\infty}\frac{\card\bigcup_{x\in\mathbb{F}_{n}(a) - \mathbb{S}}\{k\in\mathbb{N} \mid k \le \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr x\}}{\card\bigcup_{y\in\mathbb{F}_{n}(b) - \mathbb{S}}\{k\in\mathbb{N} \mid k \le \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr y\}} \\
% 		=&\lim_{N\to\infty}\frac{\sum_{x\in\mathbb{F}_{n}(a) - \mathbb{S}}^{x\le \widetilde{\log}_{n}(N;\bm{a}_{-})}\lfloor\frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr x\rfloor}{\sum_{y\in\mathbb{F}_{n}(b) - \mathbb{S}}^{y\le \widetilde{\log}_{n}(N;\bm{b}_{-})}\lfloor \frac{1}{\Phi_1} \log_{\beta}(\alpha N + \gamma) - \frac{1}{\Phi_1}\log_{\beta}\chr y\rfloor} \\
% 		=&\lim_{N\to\infty}\frac{\frac{\alpha-1}{\alpha}\widetilde{\log}_{n}(N;\bm{a}_{-})\log_{\beta}(\alpha N + \gamma) - \sum_{x\in\mathbb{F}_{n}(a) - \mathbb{S}}^{x\le \widetilde{\log}_{n}(N;\bm{a}_{-})}\log_{\beta}\chr x}{\frac{\alpha-1}{\alpha}\widetilde{\log}_{n}(N;\bm{b}_{-})\log_{\beta}(\alpha N + \gamma) - \sum_{y\in\mathbb{F}_{n}(b) - \mathbb{S}}^{y\le \widetilde{\log}_{n}(N;\bm{b}_{-})}\log_{\beta}\chr y}.
% 	\end{align*}
% 	while the reduce vector is $\bm{a} = (\val_{\beta}\chr a_{-n}, \cdots, \val_{\beta}\chr a_{-1}, \val_{\beta}\chr a)$ and.
% \end{proof}
...
\begin{proof}
	...
	We can construct a sequence of set that $A_{n} = \{x\in\mathbb{N} \mid x \le M\} - \bigcup_{k=0}^{n-1}\mathbb{F}_{k}(b)$. We have $A_{n+1} \subset A_{n}$. Thus $\{A_n\}_{n=1}^{\infty}$ has a minimal element. Suppose it's $A_m$. Thus let $ H = \min\{k\in\mathbb{N}\mid A_{k} = A_{m}\}$, $\forall n\ge H$, we have $\card\{\mathbb{F}_{n}(b)\le M\} = 0$.
...
% 	\[ \frac{\mu\mathbb{F}_{n+1}(b)}{\mu\chr\mathbb{F}_{n}(b)} = \alpha\frac{(\beta^{H(n)} - 1)\beta^{\Phi_1}}{\beta^{H(n)}(\beta^{\Phi_1} - 1)} = \frac{\alpha\beta^{\Phi_1}}{\beta^{\Phi_1} - 1}\left(1-\frac{1}{\beta^{H(n)\Phi_1}}\right). \]
...
% 	Then from \cref{cor:source-value-set-equivlants-its-pre-chr-set}, we have $\alpha\mathbb{F}_{n+1}(b) + \gamma = \mathbb{B}\otimes \chr\mathbb{F}_{n}(b)$. Thus we have
% 	\begin{align*}
% 		\mu(\alpha\mathbb{F}_{n+1}(b) + \gamma) &= \mu\mathbb{B}\otimes \chr\mathbb{F}_{n}(b) \\
% 		&= \frac{1}{N}\sum_{x\in\mathbb{B}}^{x\le N} \card\left\{\chr\mathbb{F}_{n}(b) \le \frac{N}{x} \right\} \\
% 		&= \frac{1}{N}\sum_{k=0}^{\frac{1}{\Phi_1}\log_{\beta}N} \card\left\{\chr\mathbb{F}_{n}(b) \le \frac{N}{\beta^{k\Phi_1}} \right\} \\
...
% \end{lemma}
% \begin{proof}[Proof of \cref{thm:proportional-of-density-of-source-value-set}]
% 	\[ \frac{\mu\mathbb{F}(b)}{\mu \mathbb{B}} = \frac{\alpha}{\chr b}\cdot\frac{\mu\mathbb{F}(b)}{\mu\mathbb{F}_{1}(b)} = \frac{\alpha}{\chr b}\cdot\sum_{n=0}^{\infty}\prod_{m=0}^{n}k(m). \]
...
\begin{corollary}
    ...
    \[ \widetilde{\pi}(\alpha\mathbb{F}_{n+1}(b) + \gamma;x) = \frac{1}{2\pi\rmi}\int_{c-\rmi\infty}^{c+\rmi\infty}\frac{D(\chr\mathbb{F}_{n}(b);s)}{1- \beta^{s\Phi_1}}\cdot\frac{x^{s}}{s}\rmd{s}. \]
\end{corollary}
\begin{proof}
	From \cref{cor:source-value-set-equivlants-its-pre-chr-set}, we have 
	\begin{align*}
		D(\alpha\mathbb{F}_{n+1}(b) + \gamma; s) &= D(\mathbb{B}\otimes \chr\mathbb{F}_{n}(b); s) \\
		&= D(\mathbb{B};s)\cdot D(\chr\mathbb{F}_{n}(b);s) \\
		&= \sum_{k=0}^{\infty}\beta^{ks\Phi}\cdot D(\chr\mathbb{F}_{n}(b);s) \\
		&= \frac{D(\chr\mathbb{F}_{n}(b);s)}{1 - \beta^{s\Phi}}.
	\end{align*}
	...
	\begin{align*}
		...
		&= \frac{1}{2\pi\rmi}\int_{c-\rmi\infty}^{c+\rmi\infty}\frac{D(\chr\mathbb{F}_{n}(b);s)}{1- \beta^{s\Phi_1}}\cdot\frac{x^{s}}{s}\rmd{s}.
	\end{align*}
\end{proof}

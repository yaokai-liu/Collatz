...
\begin{theorem}~\label{theorem:any-cycle}
    Given $\{v_{i}\}_{i=1}^{n}$ that $n < \infty$, if $\forall i>j\ge1, \beta \nmid \mathfrak{A}_i^{j}$, then there exists only one Collatz sequence in $\widehat{\mathbb{Q}}_{{\beta}}$ is a cycle that $\val_{\beta}(a;k) = v_{k\fmod n}$.
\end{theorem}
...
\begin{theorem}~\label{theorem:any-presequence}
    ...
    If $\forall i>j\ge1, \beta \nmid \mathfrak{A}_i^{j}$, then there exists only one Collatz sequence in $\widehat{\mathbb{Q}}_{{\beta}}[b]$ that $\val_{\beta}(a;i) = v_{i}$ and $a_n = b$. Of course, we requires $\beta \nmid b$.
\end{theorem}
\begin{proof}
